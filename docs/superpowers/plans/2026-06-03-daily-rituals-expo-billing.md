...
**Tech Stack:** _⚠️ **Historical — this plan is complete and its stack line is stale.** The app now runs **Expo SDK 54 / React Native 0.81.5 / React 19.1.0** and `react-native-purchases` **^10.5.0** (see IMP-027 and [`playbook.md`](../../playbook.md)). Read the versions below as a dated record, never as the current stack._ Expo SDK 51, React Native 0.74.5, JavaScript (no TS conversion), `react-native-purchases` (RevenueCat) v8 + its Expo config plugin, `expo-dev-client`, `expo-linear-gradient`, `react-native-svg`, Jest + `jest-expo` + `@testing-library/react-native` for the new billing logic.
