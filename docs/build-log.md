...
### IMP-076 — the app moves to the New Architecture   ·   Lane: Build   ·   Status: ✅ code-complete (2026-08-17)
...
v1.0.3 / vc9 in production on `targetSdkVersion 36`, so the hold outlived its reason by three weeks. SDK 55
removes Legacy Architecture outright, making this forced work either way.

**Landed exactly as specified — no app code changed at all**, which is what keeps the rollback one line each
way. `app.config.js` → `newArchEnabled: true`, `android/gradle.properties:38` → `newArchEnabled=true`, both
surrounding comments **rewritten rather than deleted** so the next reader sees a *superseded* IMP-027
decision (each names the shipped v1.0.3 / vc9 build as the reason the hold ended) rather than a silent
reversal. `playbook.md`'s stack block rewritten from "Architecture: Legacy, held deliberately" to the New
Arch entry, including the rollback note.

**⚠️ Only `app.config.js` is durable — `android/` is gitignored** (`.gitignore:19`, "Native Builds (Expo
Prebuild / EAS)"). The `gradle.properties` flip is what made the **local** gradle build honour New Arch; it
is not in the commit and cannot be. On EAS, prebuild regenerates `gradle.properties` from
`expo.newArchEnabled`, so the config field is the single durable switch. Anyone rebuilding this locally from
a clean checkout must re-run `expo prebuild` (or re-flip by hand) before `./gradlew`.

**`scripts/patch-permissions.js` did NOT fire, and it is NOT retired.** `npm install` exited 0; the script
reported `already null-safe — nothing to do`. Note that the flag cannot affect it either way — it inspects
`node_modules`, which an architecture flag does not change — so the real question (*does New Arch route
permissions somewhere else?*) was answered from source instead, and the answer is **no**:
- `PermissionsService.kt` lives in `expo-modules-core/android/src/main/`, not an arch-gated sourceset. The
  **only** arch-gated sourceset in that package is `src/fabric/`, which is **C++ only** (`CMakeLists.txt` +
  `FabricComponentsRegistry.{cpp,h}`) — no Kotlin/Java in it.
- It is supplied unconditionally by `ReactAdapterPackage.createInternalModules()` (line 29), consumed by
  `ModuleRegistryAdapter.createNativeModules()` (lines 63–78) with **no** `IS_NEW_ARCHITECTURE_ENABLED`
  branch. That file's single arch branch is at line 115 and is additive — Fabric **view manager**
  registration only.
- **Pristine-tarball re-verification** (per the spec, since the installed copy is what the patch rewrites and
  so cannot answer the question): `npm pack expo-modules-core@3.0.30` → `requestedPermissions!!.contains(permission)`
  still present upstream at line 174. The bug is unfixed and the patch stays correctly targeted.

**Proof — the native build, which is the point of this spec.** `./gradlew assembleRelease` →
**BUILD SUCCESSFUL in 4m 34s**, 847 tasks, exit 0, 94 MB `app-release.apk`. The APK is **verifiably** New
Arch, not merely built with the flag on: it carries `lib/*/libappmodules.so` plus
`libreact_codegen_rnsvg.so` and `libreact_codegen_safeareacontext.so` — codegen artifacts that exist only
when `newArchEnabled=true`. `react-native-purchases` and `react-native-purchases-ui` — the audit's one soft
spot — both `assembleRelease`d clean, and R8 ran over the result. Warnings seen and judged benign:
`MainApplication.kt` deprecation notices for `ReactNativeHost` (RN 0.81 deprecates the old bridge host under
New Arch), `[CXX5304]` SDK-XML-version notices, and pre-existing R8 stack-map warnings from the Amazon
appstore SDK (a RevenueCat transitive dep, unrelated to this change).

**One environment gotcha worth keeping.** The first attempt failed in 2s with
`Error resolving plugin [id: 'com.facebook.react.settings'] > 25.0.2` — **Android Studio's bundled JBR is
Java 25**, which AGP / the RN settings plugin reject. Build with **JDK 17**
(`/opt/homebrew/opt/openjdk@17/libexec/openjdk.jdk/Contents/Home`); nothing about it is New-Arch-specific.
The `~/.gradle/init.d` kapt tmpdir fix was **not** needed and that directory does not currently exist — the
issue did not resurface on the New Arch codegen path.

**Proof — the rest.** `npm test` → **867 passed, 84 suites**, plus **3 zone tests × 2 pinned zones**, exit 0
(run as `npm test`, not bare `npx jest`, so the zone half really ran). `npx expo export --platform android`
clean. ⚠️ **Neither is evidence this spec worked** — no app code changed, so jest is structurally blind to
the architecture flip, and export exercises the JS bundle rather than the native runtime. Step 3's native
build is the only mechanical proof, and correctness is settled by WALK-16.
**Bump:** `npm run bump:native` → **v1.0.7 / versionCode 13** (native change, so `version` bumps — that is
what scopes OTA to compatible builds).
**Ship:** none. No trailer, not pushed, not merged.
**Commit:** `build(arch): the app moves to the New Architecture (IMP-076)`.
**Runtime proof: [WALK-16](walk-open.md) then [WALK-17](walk-open.md)** — device rows, a different chat.
WALK-16 decides whether the migration survives; WALK-17 re-audits edge-to-edge, because IMP-027's pass was
on Legacy Arch and does not carry over. **`PLUS_ENABLED` stayed `false` throughout** — the RevenueCat soft
spot is dormant by design here, and WALK-11 is not reopened.

---

### IMP-080 — the Paywall footer stops fighting the layout   ·   Lane: Build   ·   Status: ✅ code-complete (2026-09-05)

**Branch-only, never pushed** (`feat/design-push`, no `Release-Lane` trailer). Scoped from the 🔴 WALK-07
finding, 2026-08-16 — not from the design doc.

**Why the third fix was different from the first two.** `Paywall.js`'s footer sat as the last child of a
flex column above a `flex: 1` ScrollView. Android's Modal is a Dialog whose window size is unknown on the
first measure pass, so the column resolved against nothing and the footer landed on the plan selector and
the disclaimer. **IMP-068** (`flex: 1` on the ScrollView) and **IMP-074** (`maxHeight: winH` on the root)
were both aimed at that measure pass, and WALK-07's re-run found **both present in code and the overlap
still happening from the very first frame** — not the delayed-then-correcting pass IMP-074 described. So
this spec stopped tuning the measurement and **removed the race**: the footer's position no longer depends
on the column measuring correctly at all.

**Landed exactly as specified.** Root `View`: `flex: 1, maxHeight: winH` → **`height: winH`** — an exact
viewport height, not a cap, because step 3 pins with `bottom: 0` and under a cap a *short* page would size
the root to its content and float the footer up the middle. `winH` still from `useWindowDimensions()`.
Footer `View`: `position: 'absolute', left: 0, right: 0, bottom: 0` plus
`onLayout={(e) => setFooterH(e.nativeEvent.layout.height)}`, into `useState(96)` — **seeded at the real
approximate height, not 0**, so the first frame isn't short. Its `backgroundColor: c.surface` is
load-bearing now: content scrolls *underneath* it. ScrollView `paddingBottom`: `18` → `18 + footerH`; its
`flex: 1` and IMP-068's comment both stay. `flow.overlay` stays the last child.

**Both comments rewritten rather than deleted**, naming IMP-068 and IMP-074 as **superseded rather than
wrong** — the same discipline IMP-076 used on the New Arch flags, so the next reader does not read a
silent reversal.

⚠️ **Recorded so it is not retried:** the alternative floated during the walk — don't render the footer
until a plan is picked — **does not work.** `plan` initialises to `'annual'`, so a plan is always picked.

**Tests.** The IMP-068 describe is untouched and still passes. The IMP-074 describe became
`Paywall — IMP-080 (supersedes IMP-074)`: exact `height` with `maxHeight` gone, the `useWindowDimensions`
source assertion **unchanged**, the footer's pin, the reserved `paddingBottom > 18`, and a regression test
naming the real defect — **commented with the fact that jest renders a tree, not pixels, and cannot see
the overlap**; it asserts the structural property that made the overlap possible (the footer is no longer
a flex sibling) instead.

**Green:** 868 passed / 84 suites, `npx expo export --platform android` clean. **No `bump:native`** — pure
JS. **`PLUS_ENABLED` stayed `false`.**
**Commit:** `fix(paywall): the footer leaves the flex column (IMP-080)`.
**Runtime proof: the Paywall half of [WALK-07](walk-open.md)** — a different chat.

---

### IMP-081 — the never-backed-up warning says the whole sentence   ·   Lane: Build   ·   Status: ✅ code-complete (2026-09-05)

**Branch-only, never pushed** (`feat/design-push`, no `Release-Lane` trailer). Scoped from the WALK-03
step 4 finding, 2026-09-05 (emulator, agent-run).

**Why it mattered more than a truncation.** `BackupNudge` clamped at `numberOfLines={2}`, and the two
strings it is given are not the same length: `stale` is 62 chars and fits, `never` is 97 and did not. On a
1280×2856 screen **at default font scale** it rendered "…there's nothing to bring ba…". The clause being
eaten is the consequence the warning exists to state, and the user who has never backed up is the one
person it is written for.

**Landed exactly as specified.** `numberOfLines` 2 → **3**, and **the clamp stays** — it is what stops a
future long string from pushing "General" off the card. The row went `alignItems: 'center'` →
`'flex-start'` with `marginTop: 1` on the ⚠ icon, because at three lines a centred row floats the glyph
visibly below the first line; `gap: 8`, `marginTop: 10` and `paddingHorizontal: 4` all stay. **The copy
was not shortened** — the layout was wrong, not the sentence.

**Commit:** `fix(you): the never-backed-up warning fits (IMP-081)`.
**Runtime proof: the step 4 half of [WALK-03](walk-open.md)**, `neverBackedUp` scenario, at default **and**
max font scale — max font is where 3 lines actually gets tested. A different chat.

---

### IMP-077 — a motion vocabulary the whole app can speak   ·   Lane: Build   ·   Status: ✅ code-complete (2026-09-05)
...
**What it is, and what it deliberately is not.** It adds **no animation to any screen**. It builds the one
named primitive set that later per-screen IMPs and every Claude Design spec get written in. New
[`src/motion.js`](../src/motion.js) exports `DUR` (`tap: 120, enter: 320, settle: 480, celebrate: 900`),
`EASE` (three curves plus a spring config), `riseIn(delay)`, `popIn(delay)`, `fadeOut()`,
`stagger(i, step = 60)`, `usePressScale()`, `useCountUp(value)` and `ScreenFade`. **The feel is sourced,
not invented** — `Celebration.js`'s spring pop into `Animated.stagger(150, …)` is the house motion, and
`EASE.pop` is its friction 5 / tension 80 restated in damping/stiffness so it survives the move.

**Purity is mechanical, not remembered.** The module holds no state of its own and imports nothing from
`persistence/`, `billing/`, `gamify.js` or `insights/` — that is the "no backend rewiring" constraint, and
**both properties are asserted** by `__tests__/motion.test.js` rather than left to intent.

**Adopted in exactly two places, as specified.** `PrimaryButton` takes `usePressScale()` in place of its
local press spring; `ScreenFade` wraps the screen container at `RitualsApp.js`, keyed on the active tab so
the swap reads as a transition instead of an instant substitution — **presentation only, no routing
change, no state change, no navigation library.** `ProgressBar`'s shimmer is untouched (it works, it is
native-driven, the regression risk buys nothing).

**🔒 The freeze held.** `art.js` (RayFan and NightRays — the signature, and the owner's explicit
constraint), `Celebration.js` and `Toast.js` are **byte-identical**, still on `Animated`. The ~12
`Modal animationType="slide"` sheets are unchanged — OS modal presentation is correct for them.
**Coexistence is the design, not a compromise:** new motion uses `motion.js`, working choreography is left
alone, the two run side by side.

**Packages** via `npx expo install`: `react-native-reanimated@~4.1.1` and `react-native-worklets@0.5.1`,
both SDK 54's own bundled recommendations. **[`babel.config.js`](../babel.config.js) is byte-identical and
that is deliberate** — `babel-preset-expo` auto-injects the worklets plugin and prefers it over the
Reanimated plugin, so adding either by hand risks double-application.

⚠️ **The jest mock lies, and the mock site says so at length.**
`jest.mock('react-native-reanimated', …)` no-ops **every** hook: shared values are plain objects, animated
styles are `{}`, nothing is scheduled on the UI thread. **A green suite proves the screens still render
and proves nothing whatsoever about the native side** — not that Reanimated 4 initialized, not that the
New Architecture is live under it, not that one frame moved. That trap is exactly what the WALK-16 gate
existed for, and the next reader must not have to rediscover it.

**Tests** (`__tests__/motion.test.js`) cover the pure surface only: `DUR`'s four speeds and their
ordering, `EASE`'s shape (the bezier returns a factory object, not a bare function — the assertion says
what actually distinguishes a curve from the spring config), `stagger` arithmetic, and two source
assertions in IMP-074's style — the forbidden-import graph, and no module-level state beyond `DUR`/`EASE`.

**Green:** 873 passed / 85 suites, both zone suites green, `npx expo export --platform android` clean
(bundle 3.87 → 4.95 MB — Reanimated's cost, on purpose).
**Bump:** `npm run bump:native` → **v1.0.8 / versionCode 14** (native deps, so the shipped vc13 artifact no
longer covers this tree).
**Ship:** none. No trailer, not pushed, not merged.
**Commit:** `feat(motion): a motion vocabulary the whole app can speak (IMP-077)`.
**Runtime proof: [WALK-18](walk-open.md)** — a mid-range device with real frame pacing, a different chat.
An emulator cannot settle it.
...
### The two Manage-screen truthfulness fixes (IMP-082 + IMP-083, 2026-09-06)

> Both from the same source — the owner putting a **real license-tester subscription** through the app
> after `PLUS_ENABLED` went true — and both the same defect class as the cut PDF perk and the decoupled
> ember packs: **the paid surface asserting something the app cannot back.** Neither was findable by
> reading code with Plus off, and neither is findable by jest, which runs `simService`.


### IMP-082 — RENEW_DATE stops being a runtime fallback   ·   Lane: OTA   ·   Status: ✅ code-complete (2026-09-06)
...
- **Steps:**
  - [x] 1. **RED** — a `formatRenewDate` test asserting `null` for `undefined`, `''`, and an
        unparseable string like `'not-a-date'`. Update the existing cases that expect `RENEW_DATE`.
  - [x] 2. `formatRenewDate(iso)` returns **`null`** instead of `RENEW_DATE` on both the missing and
        the `isNaN` branch. Its valid-ISO formatting is unchanged.
  - [x] 3. [`RitualsApp.js:180`](../src/RitualsApp.js#L180) → `const renewLabel = liveEntitlement ? formatRenewDate(liveEntitlement.renewISO) : null;`
  - [x] 4. **RED** — a `PlusBanner` test: with `plus` and a `renewLabel` it renders `Member · renews <that label>`;
        with `plus` and **no** `renewLabel` it renders `Member` and the string `renews` appears nowhere.
  - [x] 5. `PlusBanner` takes a `renewLabel` prop (default `null`). Line 52 becomes
        `Member · renews {renewLabel}` when truthy, otherwise the bare word `Member`. **Drop the
        `RENEW_DATE` import from `shopui.js`.**
  - [x] 6. Thread it: `YouScreen` and `Shop` each take a `renewLabel` prop and pass it to `PlusBanner`;
        `RitualsApp` passes `renewLabel={renewLabel}` to both.
  - [x] 7. `ManageSubscription` — `const renew = renewLabel;` (drop `|| RENEW_DATE`). Then:
  ...
  - [x] 8. `CancelSheet` — same: drop `|| RENEW_DATE`; when null the sentence ends at
        `…so you can cancel.` and the ` — you'll keep Plus until {renew}` clause is not rendered.
  - [x] 9. Add a comment on `RENEW_DATE` in `data.js` saying it is **mock data for the dev panel and
        fixtures only** and must never be a user-facing fallback — with a pointer to this IMP.
  - [x] 10. `npm test` green (must stay ≥ **875**, the count after commit `6590834`).
  - [x] 11. `npx expo export --platform android` clean.
...
- **Outcome (2026-09-06):** all 11 steps done as written; nothing in the spec turned out to be wrong.
  `formatRenewDate` returns `null` on both the missing and the `isNaN` branch and **no longer imports
  `RENEW_DATE` at all**; `RitualsApp`'s import of it is gone too. `PlusBanner` takes `renewLabel`
  (default `null`) and renders the bare word `Member` without it; `YouScreen` and `Shop` each thread
  the prop. `ManageSubscription` shows `Ends soon · access until then` / the bare plan label, and both
  billing footnotes drop the "until …" clause. `CancelSheet` ends at *"…so you can cancel."*.
  `data.js` carries the "mock data, never a user-facing fallback" comment naming this IMP.
  **Two new test files**: `__tests__/ui/PlusBanner.test.js` and
  `__tests__/screens/ManageSubscription.test.js` (the first tests for either component), plus the
  rewritten `formatRenewDate` cases. Each asserts the literal `12 Jun 2026` is absent, which is the
...
### IMP-083 — the manage/cancel deep link carries the product   ·   Lane: OTA   ·   Status: ✅ code-complete (2026-09-06)
...
- **Steps:**
  - [x] 1. **RED** — `manageUrl` tests: android + `plus_annual` ⇒
  ...
  - [x] 2. Add `PACKAGE_NAME` to `config.js` from `Constants.expoConfig?.android?.package` (`''` when absent).
  - [x] 3. Implement `manageUrl` in `links.js` per the rules above; `openExternal(kind, platform, opts)`
  ...
  - [x] 4. `toEntitlement` ([`revenueCatService.js`](../src/billing/revenueCatService.js)) adds
        `productId: ent.productIdentifier || null` to the object it returns. Do not alter `plan`.
  - [x] 5. `simService`'s `ent(plan)` adds `productId: plan === 'annual' ? 'plus_annual' : 'plus_monthly'`,
        so Expo Go and the dev panel walk the same path rather than a stub that cannot fail.
  - [x] 6. `RitualsApp` — `doCancel` and `doResume` ([`RitualsApp.js:242`](../src/RitualsApp.js#L242),
  ...
  - [x] 7. `npm test` green (must stay ≥ **875**).
  - [x] 8. `npx expo export --platform android` clean.
...
- **Outcome (2026-09-06):** all 8 steps done as written. `manageUrl({ platform, productId, packageName })`
  lives in `links.js`; `openExternal(kind, platform, opts = {})` calls it and the existing two-argument
  calls are untouched. `PACKAGE_NAME` reads `Constants.expoConfig?.android?.package` in `config.js`, and
  `LINKS.manageIos/manageAndroid` gained a comment naming them the **generic** fallbacks. `toEntitlement`
  is now **exported** (it had no test before — see the trap below) and carries `productId` raw, suffix and
  all: stripping is `manageUrl`'s job alone. `simService`'s `ent()` carries one too. `RitualsApp` gained a
  single `manageOpts()` helper feeding `openLink`, `doCancel` and `doResume`, so all three routes match.
  ⚠️ **Trap found while writing the tests, worth keeping:** `Constants.expoConfig` is **undefined under
  jest**, so `PACKAGE_NAME` is `''` in the suite. The spec's `manageUrl({...packageName})` signature is
  what makes this testable at all — a version reading the constant directly would have been untestable and
  would have silently degraded to the generic list. Separately, importing `revenueCatService` into a test
  fails to parse: `react-native-purchases` pulls in `@revenuecat/purchases-js-hybrid-mappings`, ESM that
  `transformIgnorePatterns` does not cover. `__tests__/billing/revenueCatService.test.js` opens with
...
## IMP-090 — the paywall stops promising a trial it cannot see (2026-09-07)

**Lane: OTA** (pure JS). **From:** WALK-19 step 3 on hardware, 2026-09-06. ✅ **Code-complete**, commit
`f38ca24`, 1017 green across all three of this sitting's specs (was 968), `npx expo export --platform
android` clean. ✅ **SHIPPED by OTA 2026-09-07** — update group `424b5a88-c993-44d7-91d6-db586ad22c32`,
runtime 1.0.9, manifest read back with a non-empty `rcAndroidKey`.

**What was observed on hardware.** Our button read `Start 7-day free trial`. Google Play's own purchase
sheet, opened from that button on the license-tester account, said **charging today** with the INR amount
and no free period. The owner has subscribed and unsubscribed on that Google account before, and a Play
free trial is **once per Google account, ever** — so Play was right and the product config was fine.

**Why the app could not have been right.** The claim was a string literal at `Paywall.js:122`, and
`getPrices()` returned exactly `priceString` and `price` per plan, dropping the rest of the package. There
was no trial data anywhere in the app's model: the CTA was not "wrong about the offer", it had never had
an offer to be right about.

**The decision, and it was NOT "read the offer and print the days."** RevenueCat's Android products do
expose the offer's free phase, but that describes **the offer, not this buyer's eligibility** — Play
decides eligibility at purchase time and Android has no equivalent of iOS's eligibility check. Reading the
offer and printing *"Start 7-day free trial"* would have reproduced exactly the same bug for every
returning subscriber. **The CTA must never name a trial it cannot confirm the buyer will get.**

**What shipped.**
1. `getPrices()` gained `trialDays` per plan — the free-phase length off the package's product, `null`
   otherwise. New exported-for-test `trialDaysFromProduct()` reads `defaultOption`, then any zero-priced
   phase in `subscriptionOptions`, then the legacy/iOS `introPrice` shape, parsing the ISO-8601 period.
   Every hop is guarded and the whole thing is wrapped: a missing or renamed SDK field yields `null`
   rather than throwing on a device and nowhere else. The existing `catch` returning `{}` stays.
2. `mergePrices` carries `trialDays` through under the rule the file already states — the constants never
   assert what the live data contradicts. No live offering means `null`, on every entry.
3. New pure `ctaLabel({ trialDays })` in `src/billing/prices.js`: `null` → `'Subscribe'`, a number →
   `'Try free, then subscribe'`. **It never interpolates the day count**, and a test iterates several
   counts asserting the label contains no digit at all.
4. `Paywall.js` renders `ctaLabel(prices[plan])`.
5. **The legal footer too, which the spec's step 4 reached by implication and which was the most binding
   of the four claims:** it opened *"Your 7-day free trial converts to …"* unconditionally — the binding
   price disclosure promising a trial the app had never fetched. The clause now appears only when the live
   offer carries one and is worded as a description of the offer (*"7 days free for new subscribers — Play
   Store confirms at purchase whether it applies to you"*), with a no-trial variant that still carries
   price, auto-renew and cancellation.
6. Same sweep on the two surfaces that only **open** the paywall and fetch no offer at all: `shopui.js`'s
   Plus banner (**"Try free"** → "See Plus", and *"7 days free."* dropped — this is the button the owner
   actually reported reading during the walk) and the onboarding Premium teaser (*"start with 7 days
   free"* and *"See Plus & start free trial"*).

**Tests** — `__tests__/billing/prices.test.js`, `__tests__/billing/revenueCatService.test.js` (its
`react-native-purchases` mock grew from a bare stub to jest.fn()s so `getPrices` and `restore` are
reachable) and `__tests__/screens/Paywall.test.js`. The three pre-existing `toEqual(FALLBACK)` assertions
were updated to carry `trialDays: null` — a deliberate contract change, not a loosened test. Source
assertions pin that the banner and the teaser name no trial.

⚠️ **Jest closes only the pure `ctaLabel` / `mergePrices` half.** The acceptance is **WALK-19 step 3** on a
device: on an account that has burned its trial, our button and Play's sheet must agree.

---

## IMP-091 — the pending escape arms off elapsed time, not a timer (2026-09-07)

**Lane: OTA** (pure JS). **From:** WALK-19 step 4c on hardware, 2026-09-06 — **IMP-088's acceptance
failing**. ✅ **Code-complete**, commit `cba53b1`. ✅ **SHIPPED by OTA 2026-09-07**, same group
`424b5a88-c993-44d7-91d6-db586ad22c32`.
...
changed. IMP-088 shipped exactly that escape by OTA (group `82bc2b16`).

**What was ruled out.** The wiring is correct and was re-read: `Paywall.js` renders `flow.overlay`, and the
hook's `overlay` passes `stuck` into `PurchaseOverlay`. Not a missing prop, and not the class of bug where
a call site builds its own overlay.

⚠️ **The spec's step 0 was a MEASUREMENT and it was NOT run.** Separating **(a)** the timer is throttled
behind Play's sheet from **(b)** the device never had the IMP-088 OTA needs the device, and no device was
attached to the session that landed this. What is on record is the walk's own pre-flight, which confirmed
1.0.9, the OTA applied (open → wait → kill → open, no data clear), and the published manifest reading
newest group `82bc2b16` — that points at **(a)** but does not settle it. **The fix is correct under (a)
and harmless under (b)**, and the re-run separates them: the escape either appears now, or the cause is
neither and the record must say so. **Do not let "IMP-088 does not work" enter the record until that
re-run has happened.**

**The prime suspect.** The only thing that armed the escape was a `setTimeout` in `run()`. Google Play's
purchase sheet is a **separate Android activity**, so while it is up our app is backgrounded — and Android
throttles background JS timers. A timer that does not run cannot arm anything, and the user is looking at
Play's sheet for the whole grace period.

**What shipped.**
1. `run()` stamps `startedAt = Date.now()` in a ref alongside the existing timer, which **stays**: it is
   correct whenever the app holds the foreground throughout, which is the restore path. `startedAt` is
   zeroed everywhere the pending phase ends, so `0` encodes "no flow is pending".
2. An `AppState` subscription re-checks on every transition to `active`: if a flow is still pending and
   the grace period is already spent in wall-clock terms, the escape arms immediately.
3. Unsubscribed on unmount alongside the existing `alive` teardown.
4. New pure `graceSpent(startedAt, now, graceMs)` so the rule is pinnable in jest.

⚠️ **IMP-088's rule is inherited verbatim and was not relaxed:** this **never asserts an outcome**. It
...
the one outcome worse than a spinner. A test pins that arming the escape leaves the flow in `pending`.

**Deliberately not done.** If the user returns to the foreground *before* the grace period is up, the
paused timer resumes and fires the normal way — the escape is not re-armed for the remainder. That is the
spec's design, and the delay it produces is within the grace period's own intent.

**Tests** — `__tests__/billing/pendingEscape.test.js`, which now mounts the hook with a never-settling
service and a spied `AppState.addEventListener`: a foreground transition after the grace period arms the
escape with **no timer having run**, one before it does not, a settled flow is never re-armed by a later
foreground, and the subscription is torn down on unmount.

⚠️ **Jest cannot see the thing that caused this** — it has no Play sheet and no Android activity
lifecycle. **WALK-19 step 4c, `network` case, on a device** is the acceptance.

---

## IMP-092 — "Nothing to restore" stops meaning "we could not check" (2026-09-07)

**Lane: OTA** (pure JS). **From:** WALK-19, 2026-09-06 — found off-script while trying to test IMP-091.
✅ **Code-complete**, commit `de5cd34`. ✅ **SHIPPED by OTA 2026-09-07**, same group
`424b5a88-c993-44d7-91d6-db586ad22c32`.
...
**The defect, provable from the code.** `revenueCatService.restore()` collapsed its whole error space into
three outcomes, and **every error not recognised as `network` or `owned` became `restore-empty`** — the one
kind that makes a positive claim about the user's account. `mapPurchaseError` returns `'failed'` for
anything it does not recognise, and that `'failed'` was silently relabelled *"We couldn't find a
subscription on this account."* One sentence meant both *"we checked, you have nothing"* and *"we could
not check"* — the **exact inverse** of the rule `getEntitlement()` states twenty lines below it, which
deliberately does not swallow its error (IMP-043).

**Why it mattered more than it looked.** The person most likely to tap Restore is a **real subscriber on a
new phone**. Telling them their subscription does not exist, when the truth is that we could not reach the
store, is the worst sentence this flow can produce, and it points them at support.

**What shipped.**
1. `restore()` stops relabelling. An unrecognised error returns `{ kind: 'failed' }`; only a *successful*
   call that finds no entitlement returns `restore-empty`. The `network` and `owned` branches are
   unchanged.
2. New pure `resultCopy(kind, mode)` gives `PurchaseOverlay` a restore-mode variant of the `failed` card —
   *"We couldn't check."* / *"Something stopped us reaching the store. Nothing has changed — try again in
   a moment."* — because `RESULT_META.failed`'s copy is purchase-shaped (*"you weren't charged"*), which is
   both irrelevant and quietly alarming on a path that never charges anyone. The result phase now carries
   `mode` so the overlay can tell the two apart.
3. **A second call site, not named in the spec but the same defect:** `RitualsApp.doRestore` — the **You
   tab's** "Restore purchases" row — toasted *"Nothing to restore"* for every non-success kind. Fixing
   `restore()` alone would have left that sentence intact on the surface a subscriber on a new phone is
   most likely to use. It now distinguishes `restore-empty` from *"Couldn't reach the store"*.

⚠️ **KNOWN LIMIT, recorded and not fixed.** An instant answer in airplane mode means
`Purchases.restorePurchases()` **resolved from RevenueCat's local cache** rather than failing. That is SDK
behaviour, not ours, and the `catch` cannot see it — so `restore-empty` stays reachable offline for a
genuine subscriber whose cache is cold. Noted at the call site in source and carried on the WALK-19 row.
Closing it would need a reachability signal the app does not currently have.

**Tests** — `__tests__/billing/revenueCatService.test.js` (an unrecognised error is `failed`; an error with
no code at all is `failed`; only a successful empty call is `restore-empty`; `network`/`owned`/a real
entitlement all unchanged; a cancelled restore is not reported as an empty account) and
`__tests__/billing/pendingEscape.test.js` for `resultCopy`, including that a failed restore never claims
the account has no subscription and that every other kind is untouched by the mode.
...
Back dismissed Google's page **and closed the entire paywall**. Waiting **30 seconds without touching
anything** did not help: Play's error page **does not self-dismiss**.

**What it broke.** The `Modal` wrapping `Paywall` closes on `onRequestClose`, unmounting the screen and
with it `usePurchaseFlow`. The pending flow was discarded silently — the unmount cleanup sets
`alive.current = false`, and **`onAbandon` never fired**, because it is called only from `dismiss()`. The
app walked away from a purchase it had started **without asking the store what happened.**

**How much it actually cost — measured, not assumed.** `useLaunchEntitlementCheck`
([`entitlementSync.js:33`](../src/billing/entitlementSync.js#L33)) runs on every launch where `plus` is
false and grants Plus if an entitlement exists. So the bad version — charged, backed out, never noticed —
**did not happen**; the user got their Plus on the next launch. **A delay and a confusing minute, not a
lost payment.** It was written up because it is why a 🚦 walk row could not close, not because it was
dangerous.

**The second cost.** It made **IMP-091 unobservable**: the escape's container is destroyed before the
Close button can be seen, so WALK-19 step 4c could not close either way.

⚠️ **The tempting fix was the wrong one, and the spec said so before a line was written.** Making Back a
no-op while a flow is pending would have kept the card alive to be looked at — and **re-created the exact
IMP-088 trap** for the first 20 seconds, during which no exit exists at all. Back is a *good* exit. The
defect was that it exited **without reconciling**, and that the card told the user not to use it.

**What shipped.**
1. `dismiss()` stops gating the reconcile on `stuck`. That guard was right when the 20-second escape was
   the only route to it; a back press at 3 seconds abandons a real purchase just as much as one at 30.
   **What matters is that a flow was in flight**, not how long the user waited. `onAbandon` stays
   failure-tolerant through `checkEntitlement`/`nextPlusState` (IMP-043) — an unreachable store still
   changes nothing rather than downgrading. A settled *result* card still reconciles nothing, unchanged.
2. `usePurchaseFlow` exposes `pending` and `dismiss`. `Paywall` publishes both to whoever owns its Modal
   through a `closeGuard` ref, set in an effect (so the caller always reads a committed render) and
   **cleared on unmount**, so a stale flow can never be abandoned. `RitualsApp`'s Modal reconciles in
   `onRequestClose` and **then closes** — a source assertion pins that it still closes, because a no-op
   there would be the original defect wearing a fix's clothes.
3. The pending card's **"Don't close the app."** is gone. Back closes it, that is safe, and it now
   reconciles — so the line was false, and it was talking the user out of the one exit that worked. New
   pure `pendingCopy(mode, platform)`: *"This can take a minute. Going back is safe — we'll check with
   Play Store either way."* Like `stuckCopy`, it **asserts nothing about the outcome**; a test pins that.

**This corrects the IMP-088 record.** That was written as *"force-quit was the only way out."* **Back
always was one.** Arguably the copy in step 3 is the larger share of what IMP-088 was really for.

⚠️ **Scope correction, recorded rather than quietly dropped.** The spec named **two** mount sites. Only
[`RitualsApp.js`](../src/RitualsApp.js) wraps `Paywall` in a `Modal`; onboarding's mount
([`Onboarding.js:67`](../src/screens/Onboarding.js#L67)) is an absolutely-positioned `View` with **no
`onRequestClose` to route**. A back press during onboarding is therefore unhandled — a separate,
pre-existing gap, not touched here and not introduced here.

**Tests** — `__tests__/billing/pendingEscape.test.js`, +14 (1017 → 1031). Behavioural: dismissing a
pending purchase calls `onAbandon` **before the escape arms**, with the right mode; dismissing with
nothing in flight, or a settled result, calls nothing; the hook exposes `pending`/`dismiss`. Copy: the
new line never says "don't close", says going back is safe, names the right store, and asserts no
outcome. Source: the guard is published and cleared, and back reconciles **then closes**. ⚠️ **The two
behavioural guards were verified to FAIL against the old `wasStuck` gate before being kept.**

**Device acceptance — and it is NOT "step 4c now shows a Close button".** Back still closes the paywall,
deliberately, so **IMP-091 stays unobservable on the Play-error path and that is the correct outcome, not
a regression.** The device half is: back out of a pending purchase and confirm the app asks the store on
the way out.
...
> ✅ **DONE the same day — see the 2026-09-07 (afternoon) note below**, which supersedes this paragraph.
...
> ⚠️ **CORRECTED 2026-09-06 (IMP-086).** This note inferred from a publish log line
> (`env: export RC_ANDROID_KEY`) that the 1.0.8 OTA "carries the key in its manifest".
> **A publish log line is not evidence — the manifest read is.** The next update on this
> lane (IMP-085, group `d5f03a47`, runtime 1.0.9) was published the same way and its
> manifest read back **`rcAndroidKey:""`**, wiping the key on every device that took it.
> The 1.0.8 group `90aa2074` was never measured, so its key status is **unknown**, not
> proven. `eas update` needs an explicit **`--environment production`**; a local `.env`
> does not save it. See IMP-086 and the release invariant in `playbook.md`.
...
### WALK-24 — one card, two grounds — ✅ CLOSED 2026-09-14 (device, owner-run)

**Target: `device`. Runner: 👤 (visual judgement).** Proves
[IMP-130](#imp-130--the-hero-card-is-one-size-whichever-sky-is-on-2026-09-14),
[IMP-131](#imp-131--the-sunburst-lost-the-numeral-2026-09-14), IMP-132 and IMP-133.

**Result — ✅ PASS, all five steps, on update `01a09cc2` (Play `internal` vc17, owner-run).**

1. ✅ **The gesture.** Switching Meteor Shower ↔ Golden Sun twice — nothing below the hero moved.
2. ✅ **The numeral and the light.** On the default sky the numeral sits inside the convergence, the disc is
   centred in the card (equal margin top/bottom, no bare band), and the rays run off every edge.
3. ✅ **The meta row.** Reads as one composition — hero above, stat row anchored at the foot — not a void.
3b. ✅ **Over Meteor Shower footage.** The meta row sits inside `SkyHero`'s bottom scrim and the XP bar still
   reads as a bar over the water.
4. ✅ **Font scale.** Nothing clipped in either shell at max font.
5. ✅ **Night mode.** Step 1's gesture identical to day; the numeral centring question holds on `NightRays`.

**IMP-130, IMP-131, IMP-132 and IMP-133 are all proven on device — no residual finding.** The row's own
named limit stands unchanged: this walk says nothing about any sky other than `meteor`, the only one with a
clip today; the 336dp ruling for skies not yet encoded remains unconfirmed until those clips exist.

**The prior record, kept for the reasoning — WALK-24 as it stood before closure:**
...
### IMP-084 — the release build stops shipping the purchase simulation (2026-09-06)

### IMP-084 — a paywall that cannot charge must not be shown   ·   Lane: BUILD   ·   Status: ✅ code-complete (2026-09-06)

- ✅ **LANDED 2026-09-06, commit `da77a7d`.** It still blocks WALK-19 until a new build ships.
...
- **Steps:**
  - [x] 1. **RED** — `easEnvironmentPreflight` tests: a production profile with
  ...
  - [x] 2. Implement `easEnvironmentPreflight` and export it; call it from `main()` so either failure
        exits non-zero. Correct the file header comment per Layer B.
  - [x] 3. `eas.json` — add the three `environment` bindings. **Change nothing else**; leave `channel`,
        `distribution` and `submit` exactly as they are.
  - [x] 4. **RED** — a test for the gate expression. Extract it as a pure exported helper
        ...
        combinations.
  - [x] 5. Implement `paywallLive`, and in `RitualsApp.js` derive `PAYWALL_LIVE` from it, then do the
        mechanical `PLUS_ENABLED` → `PAYWALL_LIVE` replacement described in Layer C.
  - [x] 6. `npm test` green (must stay ≥ **902**, the count after commit `1f4f037`).
  - [x] 7. `npx expo export --platform android` clean.
...
> build regresses to the simulation unless Layer A lands first.
  ...
  **Outcome (2026-09-06, commit `da77a7d`).** All three layers landed as specified; nothing was
  redesigned. **A —** all three `eas.json` profiles now bind an `environment` (`development`,
  `preview`, `production`); `channel`, `distribution` and `submit` untouched. **B —** new pure
  `easEnvironmentPreflight({ easJson })` next to the unchanged `billingPreflight`; `main()` runs both
  and reports every failure rather than the first. Run against the *pre-fix* `eas.json` it produced
  exactly the vc14 diagnosis — `declares environment undefined, expected "production"` — which is the
  evidence the guard works. The header comment's "hard build failure instead of a shipped one" claim
  is replaced by an account of what this script cannot see. **C —** `paywallLive({ plusEnabled,
  billingConfigured, dev })` in `src/billing/index.js`, `PAYWALL_LIVE` derived in `RitualsApp.js`
  right after `PLATFORM`, and all **16** `PLUS_ENABLED` uses swapped — props, the four `Modal
  visible=` gates and the `restoreAccess({ plusEnabled })` call. `PLUS_ENABLED` survives in that file
  only as the import and as the input to the gate, which is what the source assertion pins.

  ⚠️ **One deviation from the spec's test list, and it is a tightening not a loosening.** The
  regression assertion was first written as `/plusEnabled\s*[=:]\s*PLUS_ENABLED/`, which matched the
  gate's own legitimate `plusEnabled: PLUS_ENABLED,` line. It is now the JSX-prop form
  `plusEnabled={PLUS_ENABLED}` the spec actually names, backed by a second test that pins the full
  list of surviving `PLUS_ENABLED` lines to exactly two. That pair is stricter than one regex: any new
  use of the flag in that file fails the suite.

  **Proof:** `npm test` → **915 passed / 90 suites** (from 902/89 — one new file,
  `__tests__/billing/paywallLive.test.js`, plus four cases appended to
  `__tests__/scripts/checkBillingConfig.test.js`), both zone suites green,
...
## The 2026-09-06 billing incident — full narrative (archived from PROGRESS.md)

Kept verbatim because it is the worked example for four separate traps: a build that ships a purchase
simulation, a probe that cannot work under Metro, an OTA lane that deletes the key it should carry, and
a "clear data to reproduce" habit that guarantees you test the wrong bundle. Closed 2026-09-06.
...
> ✅ **SHIPPED by OTA 2026-09-06** — group `82bc2b16-6fe2-42b2-8198-800632d09d2a`, android update
> `01a07759-1f94-7c48-a807-f8b1466fa4b7`, runtime 1.0.9, from `5801036`. **Manifest verified: the key
> reads back `goog_…`, not `""`** — the IMP-086 discipline, applied.
>
