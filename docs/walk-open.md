...
>
> 🔄 **CORRECTED 2026-09-13 — the pre-flight target below is now WRONG.** IMP-123 shipped as a **native
> build**, not an OTA: vc17 / `1.0.10`, confirmed live on Play `internal` and installed on the owner's
> phone (`eas build:list` — build `2a2cf27c`; `eas submit:list` — submission `bb67a4d0`,
> `releaseStatus: COMPLETED`). A fresh `eas build` bundles the JS at build time, so **vc17's embedded
> bundle already carries every fix through IMP-123** — there is nothing to wait for on a second launch.
> **Take [WALK-21](#walk-21--the-first-video-sky-plays) first** (it needs vc17, which now exists); the
> Pre-flight section immediately below still checks for the vc15-era update id `01a090d8` — read the You
> tab for **`1.0.10 / vc17`** instead, and expect **`built-in bundle (no update applied)`** on the update
> line, which is correct on this build and not a stale-bundle warning. WALK-19 step 8 and WALK-12 (R8)
> still ride this same vc17 build once WALK-21 is done.
...
5. **The candle row.** ✅ [IMP-125](build-log.md#imp-125--the-candle-count-sits-with-the-week-it-protects-2026-09-13)
   is built (`d57dc2d`, 2026-09-13) and rides the same OTA as IMP-124, so the hero's bottom is **empty**
   below the XP bar — no candles, no hairline rule — and the three candles sit under the seven dots in the
   **week-strip card** instead, on ordinary theme colors. Check the hero's fixed 336dp height did not
   visibly sag or crop the footage differently.
...
### Result — ✅ PASS (the row's own checklist), 2026-09-14 (device, Play `internal`, owner-run, update `01a09b18`)

**Steps 1-3 ✅ PASS.** The subtitle, the "day streak" line and the `Lv 1 · Spark` / XP labels all read clean
over the moving water — the near-black text WALK-21 found is gone. **Step 5 ✅ PASS** — the candle row is out
of the hero and sitting under the week-strip's seven dots, on ordinary theme colors, exactly as IMP-125
...
[`HomeScreen.js:34`](../src/screens/HomeScreen.js#L34) fixes the video hero at `HERO_HEIGHT = 336` regardless
of content
([`HomeScreen.js:99`](../src/screens/HomeScreen.js#L99), `height: HERO_HEIGHT`), while the default hero
([`HomeScreen.js:107`](../src/screens/HomeScreen.js#L107)) has no explicit height and sizes to its content's
padding instead. IMP-125's own build note said `HERO_HEIGHT` was **deliberately untouched** when the candle
row was pulled out of the video hero — so the video card kept its old fixed height while its content got
shorter, and nobody had switched skies side by side on a device to notice the two cards no longer match.
**Not fixed here — a walk records, it does not repair.** Routed to an Opus session to spec, per the project's
