...
5. **The candle row — and which question this step asks depends on whether
   [IMP-125](specs-open.md#imp-125--the-candle-indicator-leaves-the-hero-card) shipped in the same OTA.**
   ✅ It is approved (owner, 2026-09-13) and is being built ahead of the ship, so expect the second form:
   - **If IMP-125 shipped:** the hero's bottom is **empty** below the XP bar — no candles, no hairline rule —
     and the three candles sit under the seven dots in the **week-strip card** instead, on ordinary theme
     colors. Check the hero's fixed 336dp height did not visibly sag or crop the footage differently.
   - **If it did not:** the three candles and the line of copy sit under a hairline rule inside the hero.
     Readable over the footage, and the rule visible as a rule.
