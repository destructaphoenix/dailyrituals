...
>
> **Every spec ends the same way:** `npm test` green (must stay ≥ the prior count, currently **545 passed,
> 56 suites**), `npx expo export --platform android` clean, commit with the **exact** message given, then
...
**The queue is empty.** IMP-045 (the last open spec) shipped code-complete 2026-08-09; its spec is archived
in [`docs/build-log.md`](build-log.md). See `PROGRESS.md` → Open items for what's next (the subscription-
track build window or the `internal` → production promotion decision).
