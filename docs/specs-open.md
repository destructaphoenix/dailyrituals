...
IMP-084 landed (commit `da77a7d`) and moved to [`build-log.md`](build-log.md). **Nothing should be
invented to fill this file** — new work comes from a 🔴 walk finding, the owner, or a design doc.
**The next task is a WALK, and the front of that queue is [WALK-19](walk-open.md)** — but it is
**blocked until a build carrying `da77a7d` reaches a track**, because Layer A cannot be OTA'd.
