...
- **Architecture: New Architecture, on since IMP-076 (2026-08-17).** `newArchEnabled: true` is set explicitly in **two** places that must stay in step — the **top level of `expo`** in `app.config.js` (the canonical SDK-54 field) and `android/gradle.properties:38`. The deprecated `expo-build-properties.android.newArchEnabled` is intentionally left unset so there is only one switch. Still explicit rather than defaulted so `expo install --fix` / `prebuild` can't flip it silently in either direction. **This supersedes the IMP-027 hold** — that deferral was coupled to the 2026-08-31 API-36 deadline, which was met on 2026-07-30 by v1.0.3 / vc9 in production, and SDK **55 drops Legacy Architecture entirely**. Rollback is both flags back to `false` plus a rebuild; IMP-076 changed no app code precisely to keep that true.
...
- [x] 10b.1b ✅ **BillDesk/Google verified the payments profile** (owner reported 2026-08-04). Subscription products could be created and activated from that point. Historical detail on the circular BillDesk/listing deadlock is in the Phase 10b notes further down.
- [x] 10b.2 ✅ **Live Play subscription products exist (annual + monthly), owner-confirmed 2026-09-05, and the base plan carries a 7-day free trial** — which is what makes the hardcoded "Start 7-day free trial" button ([`Paywall.js:118`](../src/screens/Paywall.js#L118)) truthful. ⚠️ It is still *hardcoded* and does not read the offer from RevenueCat, so if that offer is ever changed or removed in Play, this copy silently becomes a lie (the IMP-028 trap). RevenueCat production key was swapped 2026-06-06.
- [x] 10b.3 ✅ **Products attached to RevenueCat `plus` / `current`, owner-confirmed 2026-09-05.** ⚠️ **"Confirm offerings return live prices" is NOT proven** — that is runtime behaviour and needs the internal build; it is step 2 of **WALK-19**. The paywall renders whatever the offering returns (IMP-028), so an empty or misconfigured offering shows the `PLUS_PRICES` fallback constants ($4.99 / $29.99 / "Save 50%") rather than the real Play prices — quietly, with no crash.
- [x] 10b.4 ✅ **DONE 2026-09-05.** `RC_ANDROID_KEY` created as an EAS project env var (environment `production`, visibility `sensitive`) **and** as a GitHub repo secret of the same name for `release.yml`. Verified with `eas env:list --environment production`. Why it mattered: `.env` is git-ignored and never reaches EAS Build, so without this a cloud build resolves the key to `''` and silently ships the purchase *simulation*. `scripts/check-billing-config.js` now fails such a build — **but only from CI**; a local `eas build` skips it, so run the script by hand off-CI.
...
- **✅ RESOLVED 2026-09-05 — was: 🔴 HARD BLOCKER before `PLUS_ENABLED`, one of six advertised perks was not real.** The dead one was the PDF ("Your Book — export your days as a PDF", IMP-022, deferred, no code in the tree). **It was cut from `PLUS_PERKS` rather than built** (owner's call, commit `7d2e515`): five perks remain and all five are backed by shipped code. **Re-add the line only when IMP-022 lands.** The audit table below is kept because it is the record of which perk maps to which implementation — note its numbering is against the old six-item list and is now off by one past #3. Original finding, audited 2026-08-03 against `PLUS_PERKS`: (perk #6, the PDF — IMP-022, deferred; the other five are built, see the perk table above). Audited 2026-08-03 against `PLUS_PERKS` ([`data.js:144`](../src/data.js#L144)), the list the paywall sells:
