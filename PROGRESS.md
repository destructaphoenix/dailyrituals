...
| 137 | **The streak numeral is the one glyph IMP-124 left on the palette.** [`HomeScreen.js:69`](src/screens/HomeScreen.js#L69) still paints the 76px number `c.accentDeep` over footage, so Golden Hour's amber sits on Aurora's green and Starfall's violet, and the only user fix is to re-theme the whole app to correct one number. Over footage it joins the fixed white ramp `heroChrome` already defines for every other glyph in the card; off footage nothing changes. **Owner's three options answered in the spec: lock it — yes; more palette colours — no, it does not help; customisable for Plus — no, it sells a legibility bug and devalues the ember palettes.** | OTA | ⬜ **OPEN — take this first.** Spec in [`docs/specs-open.md`](docs/specs-open.md#imp-137--the-streak-numeral-stops-borrowing-the-palette-over-footage) |
| 138 | **Three of the six skies are placeholders and the owner can see all three on the phone.** `crescent` (frozen art, free, owned by default), `harvest` (a gradient and a moon glyph, 300 embers, no design) and `meteor` (a Pexels **ocean** clip in a slot named Meteor Shower) all go. `meteor`'s own build-log entry called it temporary — it existed only because it was the one `tier: 'plus'` slot reachable without a dev panel, and four real Plus skies have replaced that need. Nobody loses a purchase: `entitledId` already falls back to `classic`. ✅ **The free tier owns exactly one sky — the owner's ruling, 2026-09-16** (*"we will think about adding free stuff later"*), so Plus is the only route to a second sky. Also folds in the **Sakura Fuji day-clip swap**, unblocked the same day and verified live on R2. | OTA | ⬜ **OPEN — take second, fully unblocked** (it renumbers fixtures IMP-137 touches). Spec in [`docs/specs-open.md`](docs/specs-open.md#imp-138--the-sky-catalogue-stops-shipping-placeholders) |
...
_2026-09-16 (Opus — **the owner's first sitting with four real skies; four findings scoped, one bug diagnosed and fixed in the encode.**) — ✅ specs written, nothing built, no app code touched._

**What finished.** The owner put the skies on their phone and filed four issues. All four are now answered
in the repo rather than in a chat.

**1. The streak numeral over footage → [IMP-137](docs/specs-open.md#imp-137--the-streak-numeral-stops-borrowing-the-palette-over-footage), OTA, open.**
The diagnosis is one prop: [`HomeScreen.js:69`](src/screens/HomeScreen.js#L69) still paints the numeral
`c.accentDeep` over video, and [`heroChrome.js`](src/home/heroChrome.js)'s own header already wrote the rule
it breaks — IMP-124 moved every *other* glyph in the card onto a fixed white ramp *"that owes nothing to the
palette, because the clip owes nothing to it either"* and left the biggest one behind. The owner's three
options are answered and the answers are final: **lock it (yes)**, **more palette colours (no — any fixed
hue is still wrong over some clip)**, **customisable for Plus (no — it sells a legibility bug on the one
glyph that must never be unreadable, and a free-form picker makes the 420-ember palettes worthless)**. Also
rejected, with the reason recorded so it is not re-proposed: **per-sky accent on the numeral** — every sky
already declares one, but Emberfield's is amber over amber embers.

**2. Placeholder skies → [IMP-138](docs/specs-open.md#imp-138--the-sky-catalogue-stops-shipping-placeholders), OTA, open.**
`crescent`, `harvest` and `meteor` all go. `meteor`'s own build-log entry had already called it temporary.
No migration is needed — `entitledId` falls back to `classic`. **The one packaging question it raised was
put to the owner and answered the same day: the free tier owns exactly one sky** — *"we will think about
adding free stuff later"* — so Plus is now the only route to a second sky, and IMP-138 says plainly that it
is decided and must not be softened by leaving a placeholder in as a free row. The spec re-points six test files rather than deleting their cases, and adds the invariant that stops
this recurring: every `SHOP_SKIES` row either carries a clip with a non-empty `credit`, or is `classic`.

**3. Emberfield's freeze — diagnosed, fix applied to the clip, ⚠️ NOT PROVEN.**
Full working in [`skies-route.md`](docs/skies-route.md#emberfield-does-not-stutter--it-loops-2026-09-16).
Measured out, in order: **not a duplicate frame** (all 59 adjacent pairs score 0.860–0.885, no outlier),
**not a bad loop window** (wrap 0.98× the adjacent floor — better than the clip that shipped as `meteor`),
and 🔴 **not fixable by interpolation** — `minterpolate` to 30fps scores well and *destroys the picture*,
because **Emberfield is pixel art** and motion compensation smears the falling leaf away entirely. What is
left: it is the only sky under 24fps (**10fps, one frame = 100ms**) and was encoded `-g 48`, so 60 frames
held two keyframes and every `p.loop` restart re-decodes. `encode-sky.py` gains **`--gop`** (default
unchanged) and now warns on its own for any sub-20fps output with a long GOP; Emberfield was rebuilt
`--gop 1` — 60 keyframes, wrap 0.99× floor, 1.3 → 2.97 MB, **zero pixels changed** (0.974 SSIM against the
old encode, all of it compression). ✅ **The owner uploaded it the same day and it is verified live and
SHA-identical.** ⚠️ **Live is not proven** — it still needs a minute of watching on the phone (WALK-27
step 3). One loose end: R2's `emberfield-poster.jpg` is still the old file (0.985 against the new one — same
frame, different JPEG), harmless and not worth a trip on its own.

**4. Sakura Fuji is the night clip — ✅ resolved.** The owner had not mis-uploaded anything:
`sakurafuji-day.mp4` returned **404** and had simply never been uploaded, while the encoded day clip and its
poster had existed in `sky-build/` since 2026-09-15. Nothing needed re-encoding. **The owner uploaded both
the same day** (verified 200, 3,336,296 bytes, SHA-identical), so the one-line data edit is now **IMP-138
step 5** with no dependency. The rule that produced the block stands for the next sky: never point a
manifest row at an object you have not fetched — a missing clip is silent.

**The proof.** No app code was touched — `PROGRESS.md`, three docs and `scripts/encode-sky.py` only, so the
suite is unchanged at **1271 passed, 119 suites**. The encode claims are measured, not assumed: SSIM figures
above are from the encoded files, the 404 is a live range request, and the pixel-art finding came from
looking at frame 91 of the interpolated output, which the numbers called good.

**The exact next step.** ✅ **Nothing is owed by the owner any more** — all three asks came back the same
day: both clips uploaded and verified, and the free-tier question ruled. **A build chat takes IMP-137, then
IMP-138**, both OTA, tests-and-data only. Then
[WALK-27](docs/walk-open.md#walk-27--four-skies-and-a-white-numeral), device, owner-run, **storage cleared
first** or the Emberfield step proves nothing — expo-video caches by URL and the fix reuses the file name.
...
(device, both modes, streak 0/1/210, at max font) proves this on a screen. In parallel the owner exports the cleared sky clips into
`sky-src/`, starting with Aurora and Sakura Fuji. _(All four have since been exported, encoded and shipped —
see the 2026-09-16 note above.)_
