...
- **Steps:**
  - [ ] 1. Build `NightCrescent` (glowing gradient crescent accent + soft halo + ~5–8 gentle embers reusing `EmberSpark`); point night+v2 hero selection at it in `HomeScreen`.
  - [ ] 2. Confirm number glow + card sheen still apply for night-v2 (kept from Round 2).
  - [ ] 3. `npm test` green (≥154). Owner screenshot review on an OLED device (flip flag to `'v2'` to capture).
...
_2026-06-13 (Opus, planning) — Owner reviewed IMP-019 Round 2 (rich ember hero, commit `44bff07`) → **derailed / too busy, still not premium**. **Reverted `DARK_THEME` to `'classic'`** (commit `f3ca0e7`; updated the default-guard test; nothing had ever been OTA'd, so production was already on classic). New direction (owner pick): **crescent accent + a few embers** — a refined glowing gradient crescent (NOT the rejected flat cheese-disc moon) as a small upper-area accent, ~5–8 quiet embers, the lit streak number as hero; restrained, not a particle show. Scoped as **IMP-019 · Round 3** inline under the task block (status ⬜). **Reuse Round 2's good parts:** the streak-number text glow + the night-v2 `Card` sheen (both kept). **Flag discipline:** default stays `'classic'` while building (main always shippable); flip to `'v2'` + update the default-guard test only on owner approval, then OTA. **EXACT NEXT STEP (Sonnet):** work **IMP-019 Round 3** — build `NightCrescent` (glowing crescent accent + ~5–8 embers reusing `EmberSpark`/`EmberParticle`), point night+v2 at it, keep number glow + sheen, `npm test` green (≥154), then owner screenshot review on OLED (flip flag to `'v2'` to preview). Do NOT ship until approved._
