...
`npm test` → **1228 passed, 115 suites** (verified 2026-09-13, after IMP-124) + **3 zone tests × 2 pinned zones**.
...
| 125 | **The candle indicator leaves the hero card.** The owner's remark during WALK-21: it does not need to live inside a 336dp card already carrying a numeral, a subtitle, a level and a bar. Destination: a footer row in the **week-strip** card, where `buildWeekStrip` already draws the days a candle spent itself. | OTA | ⬜ ✅ **APPROVED 2026-09-13 — the owner chose the week-strip footer** over its own card, Shop-only, and leaving it; the spec already said so and is unchanged. **TAKE IT FIRST** — IMP-124 is committed and unshipped, and this deletes code IMP-124 just added while moving the surface WALK-22 will walk. Rejected alternatives are recorded in the spec so they are not re-litigated. `HERO_HEIGHT` **must not shrink** (every clip was cropped to 336dp) |
| 126 | **The 11th mood's bar is invisible.** Mood mix shades each bar `opacity: 1 - i * 0.1` ([`InsightsScreen.js:142`](src/screens/InsightsScreen.js#L142)); at index 10 that is **0**, past it negative. 8 built-in moods and **no cap on custom ones**, so three custom feelings in regular use render a bar that is in the data, labelled with its own count, and cannot be seen. **IMP-118's shape exactly.** | OTA | ⬜ **SPECCED 2026-09-13** — D-04's defect half, which `docs/design-queue.md` itself said to scope now regardless of the design. Clamp the floor at `0.3`, do not `slice`: index 7 already sits at `0.3`, so **no journal that exists today changes a pixel**. No walk owed |
| 127 | **The Shop's ember `+` promises an action it cannot perform.** With `EMBER_PACKS_ENABLED` false, `Shop.js:78`'s pill toasts *"Embers also gather on their own"* — an honest message behind a control that cannot do what its glyph says. | OTA | ⏸ **SPECCED 2026-09-13, sequenced after IMP-124** so two cosmetic OTAs don't land on one surface in one window. **D-09 was half wrong** — Home's pill opens the Shop and is fine; only the Shop's own pill is the defect, which is also why IMP-119's owed centring walk survives (it moves to Home's pill) |
