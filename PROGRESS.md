...
| IMP-019 | Premium dark-mode redesign — true-black AMOLED, premium hero, in line with light mode; behind a one-line revert flag (classic dark stays intact) | OTA | 🟡 dark palette/cards/number-glow/sheen ✅ (owner-approved in screenshot); hero attempts 1–3 (plain glow / busy embers / ball-moon) all rejected → **Round 4 = rays on black (reuse RayFan) scoped — WORK THIS**; default `'classic'` |
...
#### IMP-019 · Round 4 — Rays on black (reuse the light-mode hero) (owner pivot 2026-06-13)   ·   Status: ⬜  ← **WORK THIS**
...
- **Steps:**
  - [ ] 1. Add `NightRays` (compose `RayFan` + central amber bloom; optional faint embers); point night+v2 hero at it in `HomeScreen`; drop the abandoned `NightCrescent` from selection.
  - [ ] 2. Keep number glow + card sheen (already present from Round 2).
  - [ ] 3. `npm test` green (≥154). Owner screenshot review on OLED (flip flag to `'v2'` to preview).
...
_2026-06-13 (Opus, planning) — Owner screenshotted Round 3 (`NightCrescent`) → the "crescent" rendered as a **fat gibbous ball clipped off the card's right edge** (occluder `cx=284,r=36` only bit the right edge of the lit disc `cx=228,r=38` → ~58px chunk, not a sliver; geometry sat half off the 300px canvas). Crescent **abandoned**. Owner confirmed the rest of dark mode (palette, card depth, number glow, sheen) **looks good — only the hero keeps failing** (3 misses now: plain glow → busy embers → ball-moon). **Decision: stop reinventing the hero — reuse the light-mode `RayFan`** (the rotating golden sunburst the owner loves) on the black card behind the glowing number + a soft central amber bloom. Symmetric, cohesive, low-risk (proven code). Scoped as **IMP-019 · Round 4** inline under the task block (status ⬜). Discarded the owner's local `DARK_THEME='v2'` preview flip — committed default was always `'classic'`; main stays shippable. **EXACT NEXT STEP (Sonnet):** build `NightRays` = compose existing `RayFan` + a central amber bloom (optional ~4–6 faint embers; omit if cluttered); point night+v2 hero at it in `HomeScreen` (drop `NightCrescent`); keep number glow + card sheen; `npm test` green (≥154); owner screenshot review on OLED (flip flag to `'v2'`). Do NOT ship until approved; default stays `'classic'`._
