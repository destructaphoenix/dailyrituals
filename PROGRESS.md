...
> **Two open tasks left in the queue, both fully specced in [`docs/specs-open.md`](docs/specs-open.md). Recommended order — revised 2026-08-08:**
>
> **Quick + already broken in production: ✅ ALL DONE** (IMP-034, IMP-042, IMP-040 — code-complete, not yet shipped).
> **Before anyone can pay:** **IMP-043** (recoverability — the lost-phone bug) ✅ **code-complete**, **IMP-039** (candles were fake, now real streak insurance) ✅ **code-complete**, **IMP-041** (nothing explained itself, Plus was undiscoverable) ✅ **code-complete** — all three now done.
> **The retrieval track:** **IMP-035 search** ✅ **code-complete**, **IMP-036 edit/delete** ✅ **code-complete**, **IMP-037 moods** ✅ **code-complete**, **IMP-047 deeper insights** ✅ **code-complete** (makes dead perk #5 real).
> **IMP-033** (restore consent, bigger build, already settled) ✅ **code-complete.**
> **Then:** **IMP-038 "On this day"** (perk #3, unblocked by 037) → **IMP-046 Annual Recap** (perk #4; unblocked — needs 037 ✅ and 047 ✅, both done).
...
>
> Sonnet takes **one** spec per chat, in this order, unless the owner says otherwise. **`PLUS_ENABLED` must not flip until every line of the perk table is true** — IMP-039, -041 and -043 are now all done; the remaining gate is the perk table itself (see Phase 10b below).
>
...
**Current stack:** Expo SDK **54** · React Native **0.81.5** · React **19.1.0** · **Legacy Architecture** (`expo.newArchEnabled: false`, held deliberately — SDK 55 drops Legacy and that migration is its own future task) · `compileSdkVersion`/`targetSdkVersion` **36**, `minSdkVersion` **24** · `npm test` → **508 passed, 53 suites**. Details in [`docs/playbook.md`](docs/playbook.md).
...
| IMP-038 | ✨ **"On this day"** — resurface what you wrote a year / months ago. **Plus perk #3.** Unblocked (IMP-035's retrieval layer + IMP-037's mood model both done) | OTA | ⬜ **OPEN** → [spec](docs/specs-open.md#imp-038--on-this-day) |
...
| IMP-046 | ✨ **Annual Recap — "your year, remembered"** — roadmap piece C. One page per completed year: days, words, longest streak, top moods, milestones. **Plus perk #4.** Absorbs IMP-021's deferred milestone timeline | OTA | ⬜ **OPEN** — unblocked (IMP-037 ✅, IMP-047 ✅ both done) → [spec](docs/specs-open.md#imp-046--annual-recap-your-year-remembered-perk-4) |
