...
> | --- | --- |
> | a **build task** | 📍 **TAKE [IMP-135](docs/specs-open.md#imp-135--screen-cards-that-render-from-the-shipped-code) — the one open row.** The owner's question was whether Claude Design has current screenshots to build on or the code itself: **it has neither**, and this row fixes that permanently by rendering the screens from the shipped code with `react-native-web` (already a dependency, SSR path verified in plain node) instead of photographing them. **Home only** — it is the hardest screen and the rest are prop-mapping. ⚠️ **The one real risk is named in the spec and has a test**: `HomeScreen`'s ~25 props are wired in `RitualsApp.js`, so the fixture mapping is a second source of truth — the guard parses the screen's own signature and fails when a prop is added or renamed. **Tooling lane, ships nothing.** ✅ **IMP-134 is DONE** (2026-09-14, `c9dc2dc`) — tooling only, ships nothing, no OTA. `src/home/heroFrame.js` now owns `HERO_HEIGHT`/`HERO_FOCAL`/`heroReach()` (moved out of `HomeScreen.js` verbatim); the design system's frozen card renders the rays at the app's real `focal 168` / diagonal-derived `reach` — not the `focal 80 / reach 150` component defaults it was stuck on since IMP-132 — drawn in a `HERO_HEIGHT`-tall demo box per the spec's geometry, and its caption now says what is frozen (24 spokes, colour, 60s rotation, night bloom) versus what a design may move (`focal`, `reach`), dropping the false "what you see is what renders" claim. `baselinePages()` now timestamps every capture with its own mtime and says a stale one is to be deleted, not captioned — the mechanism the 14 deleted baselines never had. **1252 passed, 117 suites** (was 1249/117, +3 tests, no new suite), export clean. **The backlog is now EMPTY of anything a build chat may take** — [IMP-128](docs/specs-open.md#imp-128--apply-the-motion-vocabulary) is the only row left and it is **OWNER-GATED — do NOT take it.** ✅ **The hero-regression run (IMP-124–133) is CLOSED, SHIPPED and PROVEN on hardware** — [WALK-24](docs/build-log.md#walk-24--one-card-two-grounds--closed-2026-09-14-device-owner-run) closed 2026-09-14, device, owner-run, all five steps PASS. Full detail per row in `docs/build-log.md`; the backlog table above has the one-line summary and commit for each. |
...
`npm test` → **1252 passed, 117 suites** (verified 2026-09-14, after IMP-134) + **3 zone tests × 2 pinned zones**.
...
| 135 | **Claude Design has no picture of any screen and never had the code.** The 14 baselines are deleted, a design project holds HTML cards not `src/`, so screen-level truth only reaches it pasted inside a request. Render the screens from the shipped code with `react-native-web` (already a dependency) instead — generated cards cannot go stale, and Claude Design can edit the real screen. **Home only**; the other six are a follow-up. | tooling | ⬜ **OPEN — take this one.** Spec in [`docs/specs-open.md`](docs/specs-open.md#imp-135--screen-cards-that-render-from-the-shipped-code). Ships nothing — the app bundle is byte-identical |
