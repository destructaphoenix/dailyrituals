...
`npm test` → **1257 passed, 118 suites** (verified 2026-09-14, after IMP-135) + **3 zone tests × 2 pinned zones**.
...
| 136 | **The hero card's top third holds nothing — and it is the most-looked-at rectangle in the app.** Ports the locked D-15 design (`hero-band-v3.html`, direction A · "Sunrise"). The band reads empty not because it holds nothing but because the rays are *faintest* where there is most of them to see. Raise the **art's** focal to 96 (the numeral does not move), fade the ray tips, add a bloom behind the convergence, ramp both with the streak. **Art only — no content enters the card, and the video shells are untouched.** | OTA | ⬜ **OPEN — the first row a build chat may take.** Spec in [`docs/specs-open.md`](docs/specs-open.md#imp-136--sunrise-the-heros-empty-band-is-filled-with-light-not-content). Walk = [WALK-26](docs/walk-open.md#walk-26--the-band-is-full-of-light) |
...
[`scripts/dsCard.js`](scripts/dsCard.js) / [`scripts/fontEmbed.js`](scripts/fontEmbed.js) so both generators
share them — verified byte-identical regeneration of `design-system/` before/after. New
...
react-native-web patches, and one spec discrepancy found and logged) archived to `docs/build-log.md`.
