...
> **One spec queued, and a walk queue running alongside it. Updated 2026-08-09:**
>
> **One task is queued: [IMP-049](docs/specs-open.md#imp-049--settings-survive-a-corrupt-restore)** (OTA,
> robustness — a corrupt restore poisons the theme). Everything else in the backlog table below is ✅.
> After it, the next real work is the **subscription-track build window** (playbook 10b step B9: revive
> IMP-022 Part A, the PDF perk #6) or the still-untaken **`internal` → production promotion** decision
...
> behind its own modal) and **IMP-049** (a wrong-typed key in a restored backup poisons the theme) both
> came out of it. **Resume point: WALK-01 steps 3–9.**
>
...
| IMP-049 | 🟡 **Settings survive a corrupt restore** — `readBackup` validates the envelope but never the payload's *shape*; `mergeWithDefaults` is a shallow spread, so one wrong-typed key (proven: `settings.accent` as a string) replaces its default and every gradient in the app renders a native null | OTA | ⬜ **OPEN — first in the queue** · [spec](docs/specs-open.md#imp-049--settings-survive-a-corrupt-restore) |
