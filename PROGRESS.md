...
| IMP-019 | Premium dark-mode redesign — true-black AMOLED, premium hero, in line with light mode; behind a one-line revert flag (classic dark stays intact) | OTA | 🟡 dark palette/cards/number-glow/sheen ✅; Round 4 code ✅ (`NightRays` = RayFan + amber bloom, commit `feat(theme): night hero = rays on black`); **pending owner screenshot review** before promoting `DARK_THEME='v2'` |
...
_2026-06-13 (Round 4 code complete) — Built `NightRays` in `src/art.js`: the proven `RayFan` rotating 24-spoke sunburst (opacity 0.6 on black, slightly brighter than day) + a soft central amber bloom (radial `#fff7ea→#f59e0b→transparent`, r=80, gentle 4.2s breathe at opacity 0.22→0.38). Removed dead code: `EmberGlow`, `NightCrescent`, `EMBER_PARTICLES`, `CRESCENT_EMBERS`, `EmberParticle`, unused `Ellipse` import. `HomeScreen` now imports `NightRays`; night+v2 hero selection points at it (was `NightCrescent`). Number glow + card sheen unchanged (active for `isNightV2`). `DARK_THEME` default stays `'classic'` — flip to `'v2'` locally to preview. `npm test` → **154 passed, 20 suites** (unchanged). **NEXT STEP (owner):** Flip `DARK_THEME='v2'` in `src/theme.js`, open app in dark mode, screenshot on OLED. If approved: flip permanently + update `darkTheme.test.js` default-guard + commit (step 4) + tag `Release-Lane: ota`. If another iteration: scope Round 5._

_2026-06-13 (Opus, planning) — Owner screenshotted Round 3 (`NightCrescent`) → the "crescent" rendered as a **fat gibbous ball clipped off the card's right edge** (occluder `cx=284,r=36` only bit the right edge of the lit disc `cx=228,r=38` → ~58px chunk, not a sliver; geometry sat half off the 300px canvas). Crescent **abandoned**. Owner confirmed the rest of dark mode (palette, card depth, number glow, sheen) **looks good — only the hero keeps failing** (3 misses now: plain glow → busy embers → ball-moon). **Decision: stop reinventing the hero — reuse the light-mode `RayFan`** (the rotating golden sunburst the owner loves) on the black card behind the glowing number + a soft central amber bloom. Symmetric, cohesive, low-risk (proven code). Scoped as **IMP-019 · Round 4** inline under the task block (status ⬜). Discarded the owner's local `DARK_THEME='v2'` preview flip — committed default was always `'classic'`; main stays shippable._
