...
| IMP-019 | Premium dark-mode redesign — true-black AMOLED, premium hero, in line with light mode; behind a one-line revert flag (classic dark stays intact) | OTA | 🟡 Round 3 code complete (commit `73601fa`): `NightCrescent` (glowing crescent + ~6 embers) behind `DARK_THEME='v2'`; default stays `'classic'`; **pending owner screenshot review on OLED before promoting** |
...
#### IMP-019 · Round 3 — Crescent accent + a few embers (owner pivot 2026-06-13)   ·   Status: ⬜  ← **WORK THIS**
- **Concept (owner-chosen):** a **refined, glowing crescent moon as a small accent** — **NOT** the classic flat "cheese-disc" moon (that was rejected as a "kid's drawing") — paired with **a few** gentle embers, and the **lit streak number as the hero**. Restrained and crafted, not a particle show (the lesson from Round 2's derail). True-black AMOLED; behind `DARK_THEME='v2'`; classic byte-for-byte intact.
- **Files likely touched:** `src/art.js` (new `NightCrescent` hero; keep `NightSky` classic — and you may delete/replace the old `EmberGlow` internals since Round 2 is abandoned), `src/screens/HomeScreen.js` (point the night+v2 hero selection at `NightCrescent`; keep the number glow), `src/ui.js` (keep the night-v2 `Card` sheen), `__tests__/theme/darkTheme.test.js` (leave default-guard at `'classic'` until promotion — see below).
- **Approach (decided by Opus — restrained; intensities tune on-device):**
  1. **Refined crescent (new focal accent):** a **thin, luminous crescent** via SVG — a lit disc minus an offset "shadow" disc (or an arc) — filled with a **soft radial gradient** (bright inner edge pale-gold `#fff7ea`/`#fde68a` → dim `#f59e0b` at the outer edge) plus a **soft outer glow/halo** (low-opacity amber bloom behind it). **No craters/holes, no flat fill.** Size it as an **accent** (~70–110px) placed **offset in the upper area** (e.g. upper-right of the hero box, ~y 40–70) — **not** centered behind the number. A very gentle breathe (opacity/scale) is optional; keep it subtle.
  2. **A few embers (supporting, not the show):** **~5–8** soft glowing embers — reuse Round-2's `EmberSpark` glowing-point approach (radial-halo points, gentle rise + sway, staggered, smooth fade; **fix the `duration:1` snap**). Keep them sparse and quiet so the crescent + number read first.
  3. **Lit streak number (hero):** keep the night-v2 amber `textShadow` glow so the number is the brightest focal point.
  4. **Card sheen:** keep Round-2's night-v2 `Card` lift — good craft, retain.
  5. **Hierarchy:** number (brightest, center, y≈80) > crescent (accent, offset upper) > embers (faint). The crescent must not compete with the number; don't cluster embers over the number.
- **Keep:** number focal point y≈80; flag/revert; `PALETTES.night` + `NightSky` byte-for-byte; `useNativeDriver`; don't touch light or classic-night.
- **Promotion / flag discipline (important):** the default `DARK_THEME` stays **`'classic'`** while building, so `main` is always shippable on the known-good theme. To *see* Round 3 during dev, temporarily flip to `'v2'` locally / for screenshots, but **commit with the default left at `'classic'`**. Only when the owner approves: set `DARK_THEME='v2'`, update the default-guard in `darkTheme.test.js` (`'classic'`→`'v2'`), commit, and OTA.
- **TDD:** visual — keep `__tests__/theme/darkTheme.test.js` green (resolution + revert guards still hold). If a pure helper is extracted (crescent geometry / ember config), unit-test it.
- **Steps:**
  - [x] 1. Build `NightCrescent` (glowing gradient crescent accent + soft halo + ~6 gentle embers reusing `EmberParticle`); point night+v2 hero selection at it in `HomeScreen`. Also fixed `duration:0` → `duration:1` in `EmberParticle` rise reset.
  - [x] 2. Confirm number glow + card sheen still apply for night-v2 (kept from Round 2 — no change needed; `isNightV2` checks in `HomeScreen` and `ui.js` are still in place).
  - [x] 3. `npm test` green (154 passed, 20 suites — unchanged). **Owner screenshot review on an OLED device pending** (flip `DARK_THEME='v2'` to preview).
  - [ ] 4. **On owner approval only:** set `DARK_THEME='v2'`, update the default-guard test, commit, OTA (`Release-Lane: ota`).
- **Commit (build):** `feat(theme): crescent-accent night hero (glowing crescent + a few embers), behind DARK_THEME flag`
- **Acceptance (owner screenshots, OLED):** the dark hero shows an **elegant glowing crescent accent** (not a flat cartoon moon), the **streak number softly lit and dominant**, **a few** quiet embers, cards subtly lifted — premium and calm, clearly same-league as light mode. `DARK_THEME='classic'` still restores classic exactly.
- **Do NOT ship until owner screenshot-approves.**
...
_2026-06-13 — IMP-019 Round 3 COMPLETE (code, commit `73601fa`) — Built `NightCrescent` in `src/art.js`: (1) **thin luminous crescent** via the "carve" technique — a gradient-filled lit disc (cx=228,cy=112,r=38; pale-gold #fff7ea → amber #d97706) plus a pure-AMOLED-black occluding disc (cx=284,cy=108,r=36) that carves the right side away, leaving a thin ~18px crescent arc in the upper-right of the hero; (2) **soft ambient halo** (radial amber bloom r=68, breathes 0.18→0.32 opacity, 4.2s); (3) **crescent breathes** 0.80→0.97 opacity, same cycle; (4) **6 sparse `EmberParticle`s** (reduced from Round 2's 16 — quiet, supporting, not the show); also fixed the `duration:0` reset snap → `duration:1`. `HomeScreen.js` now imports `NightCrescent` and points the night+v2 hero selection at it (was `EmberGlow`). Number glow + card sheen unchanged (still active for `isNightV2`). `DARK_THEME` default stays `'classic'` — flip to `'v2'` locally to preview. `npm test` → **154 passed, 20 suites** (unchanged). **NEXT STEP (owner):** Flip `DARK_THEME='v2'` in `src/theme.js`, rebuild/Expo Go in dark mode, screenshot on OLED. If approved: commit with `DARK_THEME='v2'` + update the `darkTheme.test.js` default-guard + tag `Release-Lane: ota`. If another iteration needed: file as Round 4._
