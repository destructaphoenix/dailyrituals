...
| IMP-019 | Premium dark-mode redesign ("Embers in the Dark") — true-black AMOLED, ember-glow hero (no moon), in line with light mode; behind a one-line revert flag (classic dark stays intact) | OTA | 🟡 v1 too plain → **Round 2 (rich & alive ember hero) scoped — work this** |
...
#### IMP-019 · Round 2 — "Rich & alive" ember hero + premium polish (owner review 2026-06-13: v1 too plain)   ·   Status: ⬜
...
- **Steps:**
  - [ ] 1. Rebuild `EmberGlow`: layered hot-core bloom (flicker) + coal-bed glow + ~16 glowing Svg embers (sway, stagger, hero embers).
  - [ ] 2. Add the night-v2 streak-number text glow in `HomeScreen`.
  - [ ] 3. Add the night-v2 card sheen (Card component branch).
  - [ ] 4. `npm test` green (≥ 154). Owner screenshot review on an OLED device before promoting.
...
_2026-06-13 (Opus, planning) — IMP-019 dark mode reviewed by owner → **v1 too plain / not premium**; scoped **Round 2 "rich & alive"** (no code written by Opus). Diagnosis from code: `EmberGlow` is one flat amber radial blob + 5 tiny 1–2px specks (with a `duration:1` hard reset) — no hot core, no structure, no layering, so it reads dim. Owner picked the **rich & alive** direction. Round-2 spec (inline under the IMP-019 task block, status ⬜): (1) layered **hot-core bloom** (near-white core → amber → transparent) with **two out-of-phase breathe layers** for organic flicker; (2) a **coal-bed pooled glow** low in the hero; (3) **~14–18 glowing Svg embers** (radial-halo points, not solid dots) rising 60–120px with **sine sway**, staggered timing, varied sizes, 2–3 near-white "hero" embers; (4) **lit streak number** (amber `textShadow`, night-v2 only); (5) **card sheen** (faint top-lit gradient/edge highlight, night-v2 only) for premium lift. All behind `DARK_THEME='v2'`, OTA, classic byte-for-byte intact; ~16 particles max w/ `useNativeDriver`. **EXACT NEXT STEP (Sonnet):** work **IMP-019 Round 2** — rebuild `EmberGlow` + number glow + card sheen, `npm test` green (≥154), then **owner screenshot review on OLED before shipping**; do not OTA until approved (revert = `DARK_THEME='classic'`)._

_2026-06-13 — IMP-019 COMPLETE (code) — True-black AMOLED dark mode "Embers in the Dark" implemented behind `DARK_THEME` flag. Added `PALETTES.nightV2` to `src/theme.js`: canvas `#000000`, surface `#0e0e10` (neutral near-black), accentSoft `#1c160c` (crisp amber-on-black chips), muted `#8b857c`, hairline border `#26241f`, dot `#0a0a0b`, navBg `rgba(0,0,0,0.88)`. Added `export const DARK_THEME = 'v2'` + a test-seam 3rd param in `makeTheme` so both resolution paths can be unit-tested. Added `EmberGlow` hero to `src/art.js`: warm amber radial bloom (breathing scale 0.87→1.0 + opacity 0.38→0.56, 2.8 s cycle) + 5 drifting sparks (`EmberSpark`) rising and fading on pure black, different durations for natural stagger; focal point y≈80 matching IMP-003 streak center. Wired hero selection in `HomeScreen.js`: night+v2 → EmberGlow; night+classic → NightSky; day → RayFan. **Revert path:** change `DARK_THEME = 'classic'` and OTA — `PALETTES.night` and `NightSky` are byte-for-byte intact. 5 new tests in `__tests__/theme/darkTheme.test.js` (resolution guard + revert guard). `npm test` → **154 passed, 20 suites** (was 149/19 + 5 new). **⚠️ NOT yet shipped — owner screenshot review required** across Today / Reflections / You / Write in dark mode (ideally on an OLED device) before tagging Release-Lane: ota. When approved: push main — CI will OTA to production._
