...
> | --- | --- |
> | a **build task** | ✅ **IMP-124 is DONE** (2026-09-13, `87771c4`) — the hero now reads `heroChrome.js`'s ground-based colors, not the theme. 🆕 **TAKE [IMP-126](docs/specs-open.md#imp-126--the-11th-moods-bar-is-invisible) next** (D-04's defect half — an opacity floor, two characters plus a test), OTA. **[IMP-127](docs/specs-open.md#imp-127--the-shops-ember--promises-an-action-it-cannot-perform) is now buildable too** — it was gated only on IMP-124 being committed, which it now is; take it after IMP-126 so two cosmetic OTAs don't land on one surface in one window. ⏸ **[IMP-125](docs/specs-open.md#imp-125--the-candle-indicator-leaves-the-hero-card) still needs the owner's yes** — it came from an offhand remark, not a defect — and a build chat must not take it without one. |
...
| 125 | **The candle indicator leaves the hero card.** The owner's remark during WALK-21: it does not need to live inside a 336dp card already carrying a numeral, a subtitle, a level and a bar. Decided destination: a footer row in the **week-strip** card, where `buildWeekStrip` already draws the days a candle spent itself. | OTA | ⏸ **OWNER-GATED — specced but do not build without an explicit yes.** Not a defect and not tested by any walk step. The spec says why it is not a new card (D-12 already calls Home's lower half a stack with no hierarchy) and why `HERO_HEIGHT` must not shrink (every clip was cropped to 336dp) |
