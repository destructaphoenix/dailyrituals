...
| IMP-019 | Premium dark-mode redesign ("Golden Hour at Night") — warm, layered, animated amber hero, in line with light mode; behind a one-line revert flag (classic dark stays intact) | OTA | ⬜ |
...
### IMP-019 — Premium dark-mode redesign: "Golden Hour at Night" (in line with light mode, behind a revert flag)   ·   Lane: OTA   ·   Status: ⬜
- **Goal:** Replace the current flat near-black dark mode with a **warm, premium** dark theme that reads as the *same brand* as the light mode (golden-hour identity, same layout/components, the signature radial-glow hero energy) — with real depth (off-black canvas + visibly elevated cards) and an **animated amber hero**. **Hard requirement: fully revertible** to the exact existing dark design via a single flag, in case the redesign flops.
- **Why / context:** Owner feedback (2026-06-13): light mode feels premium (animated sun rays, warm cream, soft shadows) but **dark mode is a genuine downgrade**. Root causes, grounded in the code: the canvas is **pure black** (`cream: '#000000'`, [`src/theme.js:30`](src/theme.js#L30)) → flat, no depth; **drop shadows don't render on black**, so the warm-shadow card elevation that makes light mode lush is invisible at night; the **signature `RayFan`** rotating-sun animation ([`src/art.js:13`](src/art.js#L13)) is replaced at night by the quieter `NightSky` moon ([`src/art.js:43`](src/art.js#L43)), so the hero feels empty; and `accentSoft: '#2a2113'` is a **muddy** brown so chips/pills look murky. A naive light→dark color-shift was considered and **rejected** by the owner (would stay washed-out + depthless). **Owner chose Direction A — full redesign, done directly, in line with light mode, with revertability.** (This was a planning-only discussion; specs below, no code written this session.)
- **Hard constraints (do not violate):**
  ...
  - **"In line with light mode":** identical layout, spacing, radii, fonts, component structure, and copy. **Only** the palette + the night hero art + (optional) a night backdrop change. It must feel like the same app at night — the sun/day ⇄ moon/night metaphor the app already uses — not a different design.
  - Lane **OTA** (palette + SVG/JS only; the revert is also OTA).
- **Files likely touched:** `src/theme.js` (add `DARK_THEME` flag + `PALETTES.nightV2` + flag resolution in `makeTheme`; keep `night`), `src/art.js` (new `NightSkyV2` hero; keep `NightSky`), `src/screens/HomeScreen.js` (+ `Celebration.js` / anywhere the night hero renders — select art by flag), optionally a new `NightBackdrop` gradient component + the screen shell that mounts it, `__tests__/theme/darkTheme.test.js` (new — palette resolution / revert guard).
- **Approach (decided by Opus — do not re-litigate the structure; exact hexes/intensities are dial-in values to tune on-device):**
  ...
  2. **`PALETTES.nightV2` — warm, layered, golden** (starting points; tune in the screenshot loop):
     - `cream` (canvas): `#000000` → warm off-black ~ **`#14100b`**.
     - `surface` (cards): must read **elevated** above the canvas → ~ **`#221a11`** with a warm hairline `border` ~ **`#3a2f20`** (add a deeper nested-surface token if a screen needs two elevation levels).
     - `ink` `#f4eee4` (keep); `muted` a touch brighter ~ **`#a89c8a`** for legibility on the warmer ground.
     - `accent`/`accentDeep` keep amber `#f59e0b`/`#fbbf24`.
     - `accentSoft` `#2a2113` → richer warm amber-dark ~ **`#3c2c15`** so chips/pills read crisp.
     - retune `dot`, heat levels, `navBg`, `placeholder` to the new canvas (keep `heat3 = accent`).
  3. **Depth without shadows:** create elevation from **(canvas vs surface) contrast + warm hairline borders** (and optionally a subtle 1px top-edge inner highlight on cards). Leave `t.shadow()` as-is (near-no-op on dark) — don't fight it.
  4. **Carry the golden hero energy into night — `NightSkyV2`:** keep the breathing moon + twinkling stars, and **add** (a) a soft **animated amber glow/halo behind the streak number** (radial gradient, low opacity, gentle breathe), and (b) a **dimmed, slow-rotating ray fan** reusing `RayFan`'s geometry at low opacity (~0.12–0.18) in amber, so the signature radial motion the owner loves exists at night and visually ties dark to light. **Keep the art focal point at y ≈ 80px from card top** so the streak number stays centered (consistent with the existing IMP-003 tuning / the centering review).
  5. **Optional night backdrop (last step within this task):** a warm vertical gradient via `expo-linear-gradient` (already a dep) behind night screens — slightly warmer/lighter at the top → deep off-black at the bottom (a horizon glow), rendered only when `dark && DARK_THEME==='v2'`. If it risks layout regressions, ship palette + hero first and add the backdrop as the final step (still one task, one flag).
  6. **Promote only on owner approval:** build behind the flag, screenshot **Today / Reflections / You / Write / Celebration** in dark, iterate with the owner; keep `DARK_THEME='v2'` as default once approved. If it flops → flip to `'classic'`.
...
- **Steps:**
  - [ ] 1. Add `DARK_THEME` flag + `PALETTES.nightV2`; resolve in `makeTheme`; keep `night` classic intact. Add the resolution/revert test (RED → GREEN).
  - [ ] 2. Build `NightSkyV2` (moon + stars + amber glow + dimmed slow rays), focal point y≈80; keep `NightSky`. Select the hero by flag in `HomeScreen` (+ `Celebration` if it uses the night hero).
  - [ ] 3. Retune elevation — surface/canvas contrast + warm hairline borders so cards read with depth.
  - [ ] 4. (Optional, last) `NightBackdrop` warm gradient for night screens.
  - [ ] 5. `npm test` green (≥ current count + the resolution test). Owner screenshot review across all dark screens before promoting.
- **Commit:** `feat(theme): premium dark mode "Golden Hour at Night" behind DARK_THEME flag (revertible)`
- **Acceptance (owner runtime walk + screenshots):** In dark mode — the canvas is a warm off-black (not flat black); cards visibly sit **above** it (real depth); the streak hero shows an **animated amber glow + dimmed rays** around the moon (matching light mode's radial energy); chips/accents read crisp and warm; the whole thing clearly feels like the **same brand** as light mode, at night. Setting `DARK_THEME='classic'` instantly restores the **exact** original dark mode.
...
_2026-06-13 — 🚀 OTA SHIPPED — published the accumulated improvements (IMP-008 → IMP-018) to the `production` channel over the air. Closeout commit `b9d0df5` tagged `Release-Lane: ota` and pushed to `main`; CI Release workflow ran (native backstop clean — only `src/`/`__tests__/`/docs touched; `npm test` gate 149/19 green), owner gave the one-tap `production` approval, **OTA confirmed green by owner**. Since `eas update` publishes the whole current JS bundle, this single update carries everything not previously OTA'd (real zero-state, insights, onboarding-once, real dates, achievements/keepsakes, revisit-quest fix, missed-day skull, mandatory name, proportional Ember icon, afternoon greeting, same-day prefilled edit). Reaches testers on **v5+** builds only (v4 can't receive OTA). Backlog statuses flipped to "✅ shipped OTA 2026-06-13; runtime walk pending" — owner device runtime-walk of these is still the only open follow-up. Rollback if needed: Actions → Rollback OTA workflow._

_2026-06-13 (Opus, planning) — Two owner items this session. (1) **Streak-number centering check (no task filed):** verified against the code/math — horizontally the number IS centered (art SVG is `left:0,right:0,alignItems:center` so rays/moon converge on the card midline; the number block is `alignItems:center` + `textAlign:center` with symmetric 22px padding). Vertically it's tuned so the text **line-box** center lands exactly on the art focal point (card `paddingTop:26` + block `marginTop:13` + half `lineHeight:82`(41) = 80px; art container `top:-70` + SVG center 150 = 80px — matches IMP-003). Honest caveat told to owner: digit glyphs (no descenders) optically sit slightly above the line-box center, so it can *read* a few px high even though the box is centered; confirming needs a device screenshot, and the fix (if wanted) is a ~+2–4px `marginTop` nudge or baseline-anchored alignment. **No code changed; awaiting owner's go-ahead to verify on device.** (2) **Dark-mode redesign scoped as IMP-019** (planning only — no code). Owner chose **Direction A: full premium dark redesign, done directly, in line with light mode**, with a **hard revertability requirement** if it flops. Approach locked: build a new `nightV2` palette + `NightSkyV2` animated amber hero behind a single `DARK_THEME = 'v2' | 'classic'` flag in `theme.js`, keeping the existing `night` palette + `NightSky` art **byte-for-byte intact** → revert = flip one constant + OTA. Root causes diagnosed (pure-black canvas = flat; shadows invisible on black = no card depth; signature `RayFan` energy absent at night; muddy `accentSoft`). Full spec (palette starting hexes, depth-without-shadows via surface contrast + warm borders, amber glow + dimmed slow rays behind the moon at focal y≈80, optional night gradient backdrop, resolution/revert test) is inline in the IMP-019 task block. **OTA** lane. **EXACT NEXT STEP for next chat (Sonnet):** IMP backlog codeable work = **IMP-019** (IMP-006 🟡 is owner-device-only; IMP-008–018 ✅ shipped OTA 2026-06-13, owner runtime-walk pending). Work IMP-019: build behind the flag, `npm test` green (baseline 149 passed, 19 suites + the resolution test), then **owner screenshot review across Today/Reflections/You/Write/Celebration before promoting** — do not ship until owner approves; revert path is `DARK_THEME='classic'`._
