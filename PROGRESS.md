...
`npm test` → **1228 passed, 116 suites** (verified 2026-09-13, after IMP-126) + **3 zone tests × 2 pinned zones**.
...
| 127 | **The Shop's ember `+` promises an action it cannot perform.** With `EMBER_PACKS_ENABLED` false, `Shop.js:78`'s pill toasts *"Embers also gather on their own"* — an honest message behind a control that cannot do what its glyph says. | OTA | ⬜ **buildable now, no gates left.** **D-09 was half wrong** — Home's pill opens the Shop and is fine; only the Shop's own pill is the defect, which is also why IMP-119's owed centring walk survives (it moves to Home's pill) |
...
**The exact next step.** A build chat takes **IMP-127** (the Shop's inert ember `+`, `docs/specs-open.md`) —
no gates. IMP-124/125/126 all still await the owner's ship go-ahead — none carries a `Release-Lane` trailer.
