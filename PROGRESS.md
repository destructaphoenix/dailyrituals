...
> **The backlog is fully code-complete — the queue is empty. Updated 2026-08-09:**
>
> Every `IMP-xxx` row in the backlog table below is ✅. **IMP-045** (Lifetime Progress shortfall) shipped
> code-complete 2026-08-09, closing the last open item. Nothing is queued — the next real work is either
> the **subscription-track build window** (playbook 10b step B9: revive IMP-022 Part A, the PDF perk #6)
> or the still-untaken **`internal` → production promotion** decision (see Open items below).
>
> **▶️ What is actually happening right now (2026-08-09): the emulator walk of everything built since
> vc11 — IMP-033 through IMP-048, none of which has ever been run on hardware.** New defects found there
> get scoped as fresh `IMP-xxx` rows and fixed one at a time; **IMP-048** is the first (undisclosed trash
> paywall + a toast rendering behind its own modal). Walk order and the emulator techniques that make each
> item reachable are in the newest session note at the bottom of this file.
>
...
**Current stack:** Expo SDK **54** · React Native **0.81.5** · React **19.1.0** · **Legacy Architecture** (`expo.newArchEnabled: false`, held deliberately — SDK 55 drops Legacy and that migration is its own future task) · `compileSdkVersion`/`targetSdkVersion` **36**, `minSdkVersion` **24** · `npm test` → **559 passed, 57 suites**. Details in [`docs/playbook.md`](docs/playbook.md).
