...
| IMP-019 | Premium dark-mode redesign ("Embers in the Dark") — true-black AMOLED, ember-glow hero (no moon), in line with light mode; behind a one-line revert flag (classic dark stays intact) | OTA | ⬜ |
...
- **Steps:**
  - [ ] 1. Add `DARK_THEME` flag + `PALETTES.nightV2` (true black); resolve in `makeTheme`; keep `night` classic intact. Add the resolution/revert test (RED → GREEN).
  - [ ] 2. Build the `EmberGlow` hero (amber bloom + drifting sparks), focal point y≈80; keep `NightSky`. Select the hero by flag in `HomeScreen` (+ `Celebration` if it uses the night hero).
  - [ ] 3. Retune elevation — near-black surface vs `#000` + hairline borders so cards read on OLED.
  - [ ] 4. `npm test` green (≥ current count + the resolution test). Owner screenshot review across all dark screens (ideally on OLED) before promoting.
