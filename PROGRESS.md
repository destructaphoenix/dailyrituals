...
| IMP-015 | "What should we call you?" is mandatory in onboarding — can't proceed past Personalize with an empty name (new users / after reset) | OTA | ⬜ |
...
### IMP-015 — "What should we call you?" is mandatory (can't skip past Personalize blank)   ·   Lane: OTA   ·   Status: ⬜
...
- **Steps:**
  - [ ] 1. (If extracting) RED test for `isValidName`.
  - [ ] 2. Gate "Looks good" with `disabled={!name.trim()}` + add the required-hint in `Personalize`.
  - [ ] 3. `npm test` green (≥ 123, + any new).
...
_2026-06-13 (Opus, planning) — Owner filed **5 new improvements** → scoped as **IMP-014 → IMP-018** in the Improvements backlog (full inline specs: goal, why, root cause w/ file:line, decided approach, TDD, steps, commit msg, acceptance). **No app code written this session — Opus only authored the task specs.** All five are **OTA** (pure JS in `src/`). Baseline: `npm test` **123 passed, 17 suites**._

_2026-06-13 — IMP-014 COMPLETE (code) — Missed days now show a 💀 skull in both the Today week strip and the Reflections heatmap; days before the user's first-ever entry and future days remain neutral blanks. Added `minDayKey(entries)` helper to `src/home/calendar.js` (earliest `dayKey` across all entries, or `null`). Updated `buildHeatmap`: no-entry past day where `dayKey >= firstKey && !isToday` → `{ dayKey, missed: true, today: false }`; today with no entry stays `{ empty: true, today: true }`. Updated `buildWeekStrip`: past no-entry days → `'missed'` if `dayKey >= firstKey`, else `'empty'`. Updated `HomeScreen.js` `Dot`: `state === 'missed'` renders `<Text style={{ fontSize: 16 }}>💀</Text>` on the neutral dot background. Updated `ArchiveScreen.js` `Heat`: `cell.missed` renders 💀 in the dashed-border neutral cell. Also updated the header comment in `calendar.js` and updated the "ignores entries outside the 35-day window" test to match new behavior (out-of-window entry sets `firstKey` → in-window past days become `missed`). `npm test` → **131 passed, 17 suites** (was 123; 8 new cases). Commit `d1eea60`. **Acceptance (owner runtime walk — no device in session):** with a gap day (e.g. kept Mon + Wed, skipped Tue), Tue shows 💀 on both the Today week strip and Reflections heatmap; brand-new/reset user sees **no** skulls; days before first-ever entry and future days show neutral blank; today never skulled. **Known nuance:** a streak-freeze "saved" missed day still shows 💀 (no per-day freeze record). **Ship:** OTA-eligible (all changes in `src/`). **EXACT NEXT STEP:** IMP-015 — gate the onboarding "Looks good" button with `disabled={!name.trim()}` in `Personalize`._
