...
> **The retrieval track:** **IMP-035 search** ✅ **code-complete**, **IMP-036 edit/delete** ✅ **code-complete** — **NEXT: IMP-037 moods** → **IMP-047 deeper insights** (makes dead perk #5 real; needs 037).
...
**Current stack:** Expo SDK **54** · React Native **0.81.5** · React **19.1.0** · **Legacy Architecture** (`expo.newArchEnabled: false`, held deliberately — SDK 55 drops Legacy and that migration is its own future task) · `compileSdkVersion`/`targetSdkVersion` **36**, `minSdkVersion` **24** · `npm test` → **453 passed, 50 suites**. Details in [`docs/playbook.md`](docs/playbook.md).
...
| IMP-038 | ✨ **"On this day"** — resurface what you wrote a year / months ago. **Plus perk #3.** Needs IMP-037's mood model (IMP-035's retrieval layer is done) | OTA | ⬜ **OPEN** → [spec](docs/specs-open.md#imp-038--on-this-day) |
