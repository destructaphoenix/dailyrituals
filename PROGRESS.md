...
**App status — all four Play tracks, read from the Play Developer API 2026-08-13. This is the authoritative snapshot; do not re-derive it from memory of an older note.**

| Track | Active release | API level | Note |
| --- | --- | --- | --- |
| `production` (the public) | **1.0.3 / vc9** | 36 ✅ | live since 2026-07-30; carries IMP-027 |
...
- **✅ Google Play API-36 compliance (deadline 2026-08-31) is met ACCOUNT-WIDE**, not just in production. Every active release on every track is now `targetSdkVersion 36`. The banner's cause — abandoned `beta`/vc8 and `internal`/vc5 — is gone. **This blocker is closed**; 18 days of margin left on the deadline, unspent.
- The **BillDesk deadlock is ✅ UNBLOCKED**: the public Play Store URL PA-CB verification was asking for now exists.

**🔀 Build submissions go to `internal`, not `alpha` (changed 2026-08-08, commit `a299af7` — already in `eas.json` and CI; nothing to do).** `eas.json` → `submit.production.android.track` is `"internal"`, and `.github/workflows/release.yml` auto-submits there. Internal testing serves the owner only, publishes in **minutes**, and normally skips the full app review that closed testing goes through — the point being to self-test before spending a review cycle. Consequences:
- **The next build lands on `internal`, not `alpha`.** `alpha` stays at vc11 and goes stale by design. That is safe — vc11 is `targetSdkVersion 36`, so a frozen `alpha` cannot re-trigger the compliance banner.
...
**⚠️ `internal` currently serves vc9 / 1.0.3 — it is NOT the newest code.** Promoting vc9 onto `internal` fixed compliance, but it also means **installing from `internal` today gets you the 1.0.3 the public already has**, missing IMP-021/028/029/030/031 and every OTA since. The newest built code (vc11 / 1.0.5) is on `alpha`. **Self-testing on `internal` therefore requires cutting a new build** — which is what CI will do, to `internal`, on the next `Release-Lane: build` commit.

**Consequence for OTA:** `runtimeVersion` policy is `appVersion` (currently **1.0.5**), so an `eas update` today reaches **only vc11 = `alpha` testers**. Neither the public (1.0.3) nor `internal` (1.0.3) can receive it. **All six open IMP specs are OTA-lane and currently land on the `alpha` cohort alone.** Whatever `version` the next build carries becomes the runtimeVersion the OTA lane can reach from then on — `npm run bump:native` bumps it, `npm run bump:build` does not. Treat tester-visible regressions as real but contained. The app ships **free**: `PLUS_ENABLED = false`, so there is no payment surface in it at all.
...
- **✅ Phase 10a COMPLETE.** 12×14 closed-testing gate cleared 2026-07-29; production access unlocked; free release live. **✅ API-36 compliance (deadline 2026-08-31) is now met ACCOUNT-WIDE — this blocker is CLOSED (2026-08-13).** The two abandoned tracks that kept the banner firing have both been promoted to **vc9**: `beta` went 1.0.2/vc8 → **1.0.3/vc9**, `internal` went 1.0.0/vc5 → **1.0.3/vc9**. All four tracks now serve `targetSdkVersion 36`. Re-verified against the Play Developer API 2026-08-13 (track table at the top of this file). **The lesson survives the fix — see the banner-reading order below; it is the part worth keeping.**
- **⚠️ The OTA lane reaches the `alpha` cohort ONLY — and that is now a narrower audience than it sounds.** `runtimeVersion` policy is `appVersion` = **1.0.5**, which matches **only vc11**, which lives **only on `alpha`**. Since `internal` was promoted to vc9/1.0.3 for compliance, an `eas update` today reaches neither the public **nor** the internal track the owner tests on. Production stays OTA-unreachable until a build is promoted. (General rule worth internalising: **once a `bump:native` lands, the OTA lane is closed for that release until the build ships.** Land OTA-able fixes *before* the bump, or accept they ride the build.)
- **📌 The consequence for the six open IMP specs.** Every one of them is OTA-lane. They are accumulating on `main`, reachable only by `alpha`. To self-test them on `internal` — the stated reason for the track switch — they need a **build**, not an `eas update`. The cheapest path is to keep landing OTA specs, then cut one build that carries the batch to `internal` (and picks up the two standing build-lane debts: **IMP-044**'s R8 walk and, if it is ever taken, IMP-022).
